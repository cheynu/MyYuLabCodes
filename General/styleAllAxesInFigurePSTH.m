function styleAllAxesInFigurePSTH(fig)
    ...
    for i = 1:length(ax)
        ...
        ax(i).FontSize = 8; % Tick label font size
        ax(i).TickDir = 'out';
        ax(i).XLabel.FontSize = 9; % X-axis label font size
        ax(i).YLabel.FontSize = 9; % Y-axis label font size
    ...
    for j = 1:length(cb)
        ...
        cb(j).FontSize = 7; % Match axes font size
