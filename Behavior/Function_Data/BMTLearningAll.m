function BMTAll = BMTLearningAll

MEDFile = dir('*Subject*.txt');
BpodFile = dir('*MedOpto*.mat');

if length(MEDFile) ~= length(BpodFile)
    disp('check MED & Bpod file number');
    return
end

DATE = cell(1, length(BpodFile));
for i = 1:length(BpodFile)
    date = split(BpodFile(i).name, '_');
    DATE{i} = date{3};
end
...
for i = 1:length(DATE)
    date = DATE{i};
    Bpodname = ['*MedOpto*', date, '*.mat'];
    load(dir(Bpodname).name);

    MEDdir = [date(1:4), '-', date(5:6), '-', date(7:8), '*.txt'];
    MEDname = dir(MEDdir).name;
    Bname = ['B_*', date, '*.mat'];
    if isempty(dir(Bname))
        b = track_training_progress_advanced(MEDname);

    else
        load(dir(Bname).name); %#ok<*LOAD> 
    end
    b = UpdateWaitB(b); % add FP
    BMT = BMTtemp(SessionData, b, MEDname);
    BMTAll = [BMTAll; BMT]; %#ok<*AGROW> 
end

Subjname = split(BpodFile(1).name, '_');
BMTname = ['BpodMEDTable_', Subjname{1}, '_', DATE{1}, '-', DATE{end}];
% save(BMTname, "BMTAll");
writetable(BMTAll, [BMTname, '.csv']);
end

function BMT = BMTtemp(SessionData, b, MEDname)
...
for i =1:SessionData.nTrials
    ...
    elseif iStates.WaitForPress(end) == iStates.WaitForPortExit(1)
        iPressTime = 0;
    elseif isfield(iEvents, 'AnalogIn1_1') && ismember(iStates.WaitForMedTTLStim(1), iEvents.AnalogIn1_1)
        iPressTime = iStates.WaitForMedTTLStim(1);
    end
...
ind_notnan       =      ~isnan(IndBpodInMed);
tPress           =      tPress(ind_notnan);
tRelease         =      tRelease(ind_notnan);
tTrigger         =      tTrigger(ind_notnan);
IndBpodInMed     =      IndBpodInMed(ind_notnan);
...
SessionData.nTrials = length(IndTrials);
...
MEDFileName      =      repmat(MEDname, SessionData.nTrials, 1);
...
BMT        =     table(RatName, Date, TrialsBpod, IndBpod2MED, FPs, Performance, ...
