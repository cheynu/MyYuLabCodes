function LesionGroupAnalysis(GroupArray)
...
CorrectScore              =           zeros(length(FPs)+1, Nrat, length(PeriLesionLabels)); 
PrematureScore         =           zeros(length(FPs)+1, Nrat, length(PeriLesionLabels));
LateScore                   =           zeros(length(FPs)+1, Nrat, length(PeriLesionLabels));
RTLoose                     =           zeros(length(FPs)+1, Nrat, length(PeriLesionLabels)); 
RTStrict                     =           zeros(length(FPs)+1, Nrat, length(PeriLesionLabels)); 
...
RTAccum       =       cell(Nrat, length(FPs)); % Col1, Press duration(pre), Col2, Press duration (post), Col3,  FP ( 500, 1000, 1500) warmup not included. 
N_included     =       500;
...
%% Plot correct
ha1 =  axes('unit', 'centimeters', 'position', [2 row_height(1) 4 3], 'nextplot', 'add', 'xlim', [PeriLesion(1)-0.5 PeriLesion(end)+0.5], 'ylim', [40 100],...
...
line([-.5 -.5], [40 100], 'color', [0.5 0.5 0.5], 'linestyle', '--', 'linewidth', 1)

ylabel('(%)')
title('Correct')
text(2, 45, sprintf('N= %2.0d', size(LesionedRats, 1)))
...
%% Plot premature 
ha2 =  axes('unit', 'centimeters', 'position', [7.5  row_height(1) 4 3], 'nextplot', 'add',  'xlim', [PeriLesion(1)-0.5 PeriLesion(end)+0.5], 'ylim', [0 60],...
    'ytick', [0:20:100], 'xtick', [PeriLesion(1):PeriLesion(end)], 'xticklabels', PeriLesionLabels, 'ticklength', [0.02, 1],  'XTickLabelRotation', 0);
...
line([-.5 -.5], [0 60], 'color', [0.5 0.5 0.5], 'linestyle', '--', 'linewidth', 1)
...
% hcbar2.Label.String = 'Z score';
 ...
 xlabel('Sessions ')
%% Plot late 
ha3 =  axes('unit', 'centimeters', 'position', [13  row_height(1) 4 3], 'nextplot', 'add',  'xlim', [PeriLesion(1)-0.5 PeriLesion(end)+0.5], 'ylim', [0 60],...
    'ytick', [0:20:100], 'xtick', [PeriLesion(1):PeriLesion(end)], 'xticklabels', PeriLesionLabels, 'ticklength', [0.02, 1],  'XTickLabelRotation', 0);
...
ha4 =  axes('unit', 'centimeters', 'position', [19.5  row_height(1) 4 3], 'nextplot', 'add',  'xlim', [PeriLesion(1)-0.5 PeriLesion(end)+0.5], 'ylim', [200 1000],...
...
ha5 =  axes('unit', 'centimeters', 'position', [26  row_height(1)+2 4 2], 'nextplot', 'add', ...
    'xlim', [0 1000], 'ylim', [0 5],...
    'ytick', [0:5], 'xtick', [0:200:2000], 'xticklabel', [], 'ticklength', [0.02, 1], ...
    'XTickLabelRotation', 0);
