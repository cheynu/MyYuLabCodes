...
% check ToneTime length (there's bug that number of tone was less than press/release
ind_tone=find(Time_events(:, 2)==11);
time_tone= Time_events(ind_tone, 1);
nBothHave = find(time_tone(end)>=time_leverpress,1,'last');
if nBothHave<length(time_leverpress)
    time_leverpress = time_leverpress(1:nBothHave);
    time_leverrelease = time_leverrelease(1:nBothHave);
end
