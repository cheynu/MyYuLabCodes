classdef BehaviorClass
    ...
    methods
        function obj = BehaviorClass(bdata)
            ...
            obj.ReactionTime = [];
            obj.ToneTime = [];
            if isempty(bdata.FPs) 
                    bdata = UpdateWaitB(bdata); % add FP
                    bdata.FPs = round(bdata.FPs);
                    % bc.MixedFP = unique(bc.FP);
             end
            obj.FP = bdata.FPs;
            
            % if contains(obj.Protocol,'Probe')
            %     obj.Cue               =       bdata.Cue;
            % else 
            %     obj.Cue = isnan(bdata.Cue);
            % end
            ...
            switch bdata.Metadata.Experiment
                case {'Saline'}
                        ...
                        obj.Dose = 0;
                case {'0.25DCZ', '0.5DCZ', '1DCZ', '0.125DCZ' '0.0625DCZ'}
                        obj.Treatment = 'DCZ';
                ...
                % 2023/02/18 WXN
                 case {'0'}
                        obj.Treatment = 'Saline';
                        obj.Dose = 0;
                 case {'0.25', '0.5', '1', '0.125', '0.0625'}
                        obj.Treatment = 'DCZ';
                        obj.Dose = str2num(bdata.Metadata.Experiment);
                 otherwise
                        obj.Treatment = 'NaN';
                        obj.Dose = 0;
            end;
        ...
        function value = get.IQR_RT(obj)
            IQR_Out = NaN*ones(1, length(obj.MixedFP));
            for i =1:length(IQR_Out)
                IndSelected                                      =          find(ismember(obj.FP, obj.MixedFP{i}) & obj.Stage ==1& ~strcmp(obj.Outcome, 'Dark'));
                rt                                          =          obj.ReactionTime(IndSelected);%- obj.PressTime(IndSelected);
                if ~isempty(rt)
                    IQR_Out(i)                                        =            diff(prctile(rt, [25 75]));
                else
                    IQR_Out(i) = NaN;
                end;
            end;
        ...
        function value = get.PDF_RT(obj)         % this is the strict version, only counting correct responses
            ...
            for i =1:length(PDFOut)
                RT_Cue                                          =          obj.ReactionTime(ismember(obj.FP, obj.MixedFP{i}) & obj.Stage ==1& strcmp(obj.Outcome, 'Correct'));
                if ~isempty(RT_Cue)
        ...
        function value = get.PDF_RTLoose(obj)         % this is the loose version,  counting both correct and late responses
            PDFOut = cell(1, length(obj.MixedFP));
            for i =1:length(PDFOut)
                RT_Cue                                          =          obj.ReactionTime(ismember(obj.FP, obj.MixedFP{i}) & obj.Stage ==1 &  (strcmp(obj.Outcome, 'Correct') | strcmp(obj.Outcome, 'Late')));
                if ~isempty(RT_Cue)
                    PDFOut{i}                                        =            ksdensity(RT_Cue, obj.RTbinEdges, 'function', 'pdf');
                else
                    PDFOut{i} = [];
        ...
        function value = get.CDF_HoldT(obj)
            CDFOut = cell(1, length(obj.MixedFP)+1);
            for i =1:length(CDFOut)-1
                IndSelected                                      =          find(ismember(obj.FP, obj.MixedFP{i}) & obj.Stage ==1& ~strcmp(obj.Outcome, 'Dark')& ~strcmp(obj.Outcome, 'NAN'));
            ...
            if find(obj.Cue==0)
                IndSelected                                      =          find(obj.Stage ==1 & strcmp(obj.Outcome, 'NAN'));
                HoldTime                                          =          obj.ReleaseTime(IndSelected)- obj.PressTime(IndSelected);
                CDFOut{end}                                        =            ksdensity(HoldTime, obj.HoldTbinEdges, 'function', 'cdf');
            end
            value = CDFOut;
        end;

        function value = get.PDF_HoldT(obj)
            PDFOut = cell(1, length(obj.MixedFP)+1);
            for i =1:length(PDFOut)-1
                IndSelected                                      =          find(ismember(obj.FP, obj.MixedFP{i}) & obj.Stage ==1& ~strcmp(obj.Outcome, 'Dark')& ~strcmp(obj.Outcome, 'NAN'));
                HoldTime                                          =          obj.ReleaseTime(IndSelected)- obj.PressTime(IndSelected);
                if ~isempty(HoldTime)
            ...
            if find(obj.Cue==0)
                IndSelected                                      =          find(obj.Stage ==1 & strcmp(obj.Outcome, 'NAN'));
                HoldTime                                          =          obj.ReleaseTime(IndSelected)- obj.PressTime(IndSelected);
                PDFOut{end}                                        =            ksdensity(HoldTime, obj.HoldTbinEdges, 'function', 'pdf');
            end
            value = PDFOut;
        end;
        ...
        function value = get.Anticipation(obj)
            HoldTime = obj.ReleaseTime - obj.PressTime;

            datain   = cell(1, length(obj.MixedFP));
            for i = 1 : length(obj.MixedFP)
                datain{i} = HoldTime(obj.Stage==1 & ismember(obj.FP, obj.MixedFP{i}));
            end
            value = Gfunction(datain, obj.MixedFP.*0.001);
        end    
        ...
        function value = get.MixedFP(obj)
            FPtype = unique(obj.FP);
            value = [{FPtype(FPtype < 1000)}, {FPtype(FPtype >= 1000 & FPtype < 1500)},{FPtype(FPtype == 1500)}];
        end
        ...
        function value = get.Performance(obj)
            ...
            for i = 1:length(obj.MixedFP)
                n_correct                =       sum(ismember(obj.FP, obj.MixedFP{i}) & strcmp(obj.Outcome, 'Correct'));
                n_premature          =       sum(ismember(obj.FP, obj.MixedFP{i}) & strcmp(obj.Outcome, 'Premature'));
                n_late                      =       sum(ismember(obj.FP, obj.MixedFP{i}) & strcmp(obj.Outcome, 'Late'));
        ...
        function value = get.AvgRT(obj)
            ...
            for i = 1:length(obj.MixedFP)
                iFP = obj.MixedFP{i};
                ind_press = find(ismember(obj.FP, obj.MixedFP{i}) & strcmp(obj.Outcome, 'Correct'));
                FPs                   =           obj.FP(ind_press);
                HoldDurs = 1000*(obj.ReleaseTime(ind_press) - obj.PressTime(ind_press)); % turn it into ms
                % HoldDur = obj.ReactionTime(ind_press);
        ...
        function value = get.AvgRTLoose(obj)
            ...
            for i = 1:length(obj.MixedFP)
                iFP = obj.MixedFP{i};
                ind_press = find(ismember(obj.FP, obj.MixedFP{i}) & (strcmp(obj.Outcome, 'Correct') | strcmp(obj.Outcome, 'Late')));
                FPs                   =           obj.FP(ind_press);
                HoldDurs = 1000*(obj.ReleaseTime(ind_press) - obj.PressTime(ind_press)); % turn it into ms
                iRTOut = calRT(HoldDurs, FPs, 'Remove100ms', 1, 'RemoveOutliers', 1, 'ToPlot', 0, 'Calse', 0);
                N_press(i) = length(HoldDurs);
...
%             if find(obj.Cue==0)
                if isempty(obj.Treatment)
                    uicontrol('Style', 'text', 'parent',obj.Fig1, 'units', 'normalized', 'position', [0.02 0.95 1.0 0.04],...
                        'string', [obj.Subject ' | ' obj.Session ' | ' obj.Protocol], 'fontweight', 'bold', 'backgroundcolor', [1 1 1], 'fontsize', 10);
                else
                    uicontrol('Style', 'text', 'parent',obj.Fig1, 'units', 'normalized', 'position', [0.02 0.95 1.0 0.04],...
                        'string', [obj.Subject ' | ' obj.Session ' | ' obj.Protocol ' | ' obj.Treatment], 'fontweight', 'bold', 'backgroundcolor', [1 1 1], 'fontsize', 10);
                end;
%             else
%                 uicontrol('Style', 'text', 'parent',obj.Fig1, 'units', 'normalized', 'position', [0.25 0.95 0.7 0.04],...
%                     'string', [obj.Subject ' | ' obj.Session ' | ' obj.Protocol], 'fontweight', 'bold', 'backgroundcolor', [1 1 1], 'fontsize', 10);
%             end
            ...
            title(sprintf('warm-up|%d',sum(obj.Stage == 0)),"FontName",'dejavu sans','FontWeight','bold')
            axis 'auto x'
            ...
            for k = 1:length(obj.MixedFP)
                ...
                title(sprintf('%2.1f s|%d', obj.MixedFP(k)/1000, sum(obj.Cue == 1 &obj.FP == obj.MixedFP(k))),"FontName",'dejavu sans','FontWeight','bold')
                ...
                boxplot(this_rt)
                title(sprintf('%d|%d',round(median(this_rt)),round(iqr(this_rt))),"FontName",'dejavu sans','FontWeight','bold')
            ...
            title(sprintf('%s|%d', 'probe',sum(obj.Cue == 0)),"FontName",'dejavu sans','FontWeight','bold')
            ...
            title(sprintf('%s|%d','dark',sum(ind_dark_presses)),"FontName",'dejavu sans','FontWeight','bold')
