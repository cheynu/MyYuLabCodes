...
%         Cue % is always 1 for SRRT (just cut it out then)
    ...
    properties (Dependent)
        ...
        CDF_HoldT_Control
        CDF_HoldT_Chemo
        PDF_HoldT_MedianCI95_Control
        PDF_HoldT_MedianCI95_Chemo
        CDF_HoldT_MedianCI95_Control
        CDF_HoldT_MedianCI95_Chemo
    ...
    methods
        function obj = BehaviorGroupClass(SRTClassAll)
            ...
            % obj.Cue                                      =               cell2mat(cellfun(@(x)x.Cue, SRTClassAll, 'UniformOutput',false));
            ...
            obj.WaitSessions                                   = cellfun(@(x)x.Protocol(strfind(x.Protocol,'Wait')+4), SRTClassAll, 'UniformOutput', false)';
            obj.WaitSessions                   =  cellfun(@str2double, obj.WaitSessions); 
            obj.NumSessions                               =               length(SRTClassAll);
            obj.NumTrialsPerSession                  =                cellfun(@(x)x.TrialNum, SRTClassAll);
            obj.NumCorrectTrialsPerSession           =                 cellfun(@(x)x.Performance.N_press(end).*x.Performance.CorrectRatio(end).*0.01, SRTClassAll);
            ...
            obj.MixedFP                                       =                SRTClassAll{end}.MixedFP;
            % obj.MixedFP                                       =                unique(cell2mat(cellfun(@(x)x.MixedFP, SRTClassAll, 'UniformOutput',false)));

            % if find(obj.Cue==0)
            %     obj.ProbeType = length(obj.MixedFP)+1;
            % else
            %     obj.ProbeType = length(obj.MixedFP);
            % end
            rng(0)
            ...
            obj.PDF_HoldT_Sessions                  =               cell(obj.NumSessions, length(obj.MixedFP)+1);
            obj.CDF_HoldT_Sessions                  =               cell(obj.NumSessions, length(obj.MixedFP)+1);
            ...
            for i =1:obj.NumSessions
                ...
                obj.IQR_Sessions(i, :)                   =           SRTClassAll{i}.IQR_RT;
            ...
            % obj.Anticipation = cell2mat(cellfun(@(x)x.Anticipation(:,2)', SRTClassAll, 'UniformOutput', false)')'; 
            % obj.AnticipationBin = SRTClassAll{end}.Anticipation(:,1);
        end
...
%         end;
        ...
        function value = get.HoldT_Control(obj)
            HoldT                               =        cell(1, length(obj.MixedFP)+1);
            for k = 1:length(obj.MixedFP)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsControl)) & obj.Stage == 1 & (~strcmp(obj.Outcome, 'Dark')') & ismember(obj.FP, obj.MixedFP{k}) & (~strcmp(obj.Outcome, 'NAN')');
                HoldT{k}                           =       obj.HoldTime(IndexControlTrials);
            end;
            if find(obj.Cue==0)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsControl)) & obj.Stage == 1 & (strcmp(obj.Outcome, 'NAN')');
                HoldT{end}                           =       obj.HoldTime(IndexControlTrials);
            end
            value = HoldT;
        end;
        ...
        function value = get.HoldT_Chemo(obj)
            HoldT                               =        cell(1, length(obj.MixedFP)+1);
            for k = 1:length(obj.MixedFP)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsChemo)) & obj.Stage == 1 & (~strcmp(obj.Outcome, 'Dark')') & ismember(obj.FP, obj.MixedFP{k}) & (~strcmp(obj.Outcome, 'NAN')');
                HoldT{k}                           =       obj.HoldTime(IndexControlTrials);
            end;
            if find(obj.Cue==0)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsChemo)) & obj.Stage == 1 & (strcmp(obj.Outcome, 'NAN')');
                HoldT{end}                           =       obj.HoldTime(IndexControlTrials);
            end
            value = HoldT;
        end;
        ...
        function value=get.PDF_HoldT_Control(obj)
            ...
            PDF = zeros(length(obj.HoldTbinEdges), obj.ProbeType);
            for k = 1:obj.ProbeType
        ...
        function value=get.CDF_HoldT_Control(obj)
            ...
            CDF = zeros(length(obj.HoldTbinEdges), obj.ProbeType);
            for k = 1:obj.ProbeType
        ...
        function value=get.PDF_HoldT_MedianCI95_Control(obj)
                PDF_CI95 = cell(1,obj.ProbeType);
                for k = 1:obj.ProbeType
%                     HoldT_strp = bootstrp(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'pdf'),obj.HoldT_Control{k});
%                     PDF_CI95{k} = bootci(1000, @mean,HoldT_strp);
                    PDF_CI95{k} = bootci(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'pdf'),obj.HoldT_Control{k});
                end;
        ...
        function value=get.CDF_HoldT_MedianCI95_Control(obj)
                CDF_CI95 = cell(1,obj.ProbeType);
                for k = 1:obj.ProbeType
%                     HoldT_strp = bootstrp(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'cdf'),obj.HoldT_Control{k});
%                     CDF_CI95{k} = bootci(1000, @mean,HoldT_strp);
                      CDF_CI95{k} =  bootci(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'cdf'),obj.HoldT_Control{k});
                end;
                value = CDF_CI95;
                
        end

        function value=get.PDF_HoldT_MedianCI95_Chemo(obj)
                PDF_CI95 = cell(1,obj.ProbeType);
                for k = 1:obj.ProbeType
%                     HoldT_strp = bootstrp(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'pdf'),obj.HoldT_Chemo{k});
%                     PDF_CI95{k} = bootci(1000, @mean,HoldT_strp);
                    PDF_CI95{k} = bootci(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'pdf'),obj.HoldT_Chemo{k});
                end;
                value = PDF_CI95;
                
        end

        function value=get.CDF_HoldT_MedianCI95_Chemo(obj)
                CDF_CI95 = cell(1,obj.ProbeType);
                for k = 1:obj.ProbeType
%                     HoldT_strp = bootstrp(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'cdf'),obj.HoldT_Chemo{k});
%                     CDF_CI95{k} = bootci(1000, @mean,HoldT_strp);
                    CDF_CI95{k} = bootci(1000, @(x)ksdensity(x, obj.HoldTbinEdges, 'function', 'cdf'),obj.HoldT_Chemo{k});
                end;
                value = CDF_CI95;
...
%         end;
        ...
        function obj = PlotChemoEffect(obj, ind)
            ...
            uicontrol('Style', 'text', 'parent', obj.Fig5, 'units', 'normalized', 'position', [0.15 0.95 0.5 0.04],...
                'string', [obj.Subject{1} ' | Control vs ' num2str(obj.DoseSessions(strcmp(obj.Dates, obj.SessionChemo(1)))) 'DCZ'], 'fontweight', 'bold', 'backgroundcolor', [1 1 1], 'fontsize', 8, 'horizontalalignment', 'left');
            ...
            htl2.Position(2)=htl2.Position(2)+250;
             ...
             for k =1:length(IndControlSession)
                 ...
                 for i =1:length(obj.MixedFP)
                     ...
                     iIndControlTrials = IndControlTrials(ismember(obj.FP(IndControlTrials), obj.MixedFP(i)));
                     symbolSize = 5+15*(i-1);
                     % plot correct
                ...
                % Chemo trials
                for i =1:length(obj.MixedFP)
                    iIndChemoTrials = IndChemoTrials(ismember(obj.FP(IndChemoTrials), obj.MixedFP(i)));
                    symbolSize = 5+15*(i-1);
        ...
        function obj = CalRTChemo(obj)
           ...
           ChemoSession_IQR_Loose = zeros(length(obj.MixedFP)+1, 1);
            ...
            for k = 1:length(obj.MixedFP)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsControl)) & obj.Stage == 1 & (strcmp(obj.Outcome, 'Correct')') & ismember(obj.FP, obj.MixedFP{k});
                RT_Control_Strict           =       obj.ReactionTime(IndexControlTrials);
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsControl)) & obj.Stage == 1 & (strcmp(obj.Outcome, 'Correct')' | strcmp(obj.Outcome, 'Late')') & ismember(obj.FP, obj.MixedFP{k});
                ...
                IndexChemoTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsChemo)) & obj.Stage == 1 & (strcmp(obj.Outcome, 'Correct')') & ismember(obj.FP, obj.MixedFP{k});
                RT_Chemo_Strict           =       obj.ReactionTime(IndexChemoTrials);
                IndexChemoTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsChemo)) & obj.Stage == 1 & (strcmp(obj.Outcome, 'Correct')' | strcmp(obj.Outcome, 'Late')') & ismember(obj.FP, obj.MixedFP{k});
...
%        
        function value = get.PDF_HoldT_Progress(obj)
            ...
            for i =1:length(obj.MixedFP)
                IndexTrials_iFP = find(obj.Stage ==1 & ismember(obj.FP, obj.MixedFP{i})  & ~strcmp(obj.TreatmentTrials', 'DCZ'));
                % Early
...
%         end;
        ...
        function PlotPerformance(obj)
            ...
            WaitGap =find(obj.WaitSessions==2,1)-0.5;
            ...
            set(ha_HoldT_Distribution,  'units', 'centimeters', 'position', [xlevel, ylevel4, [plotsize1(1) plotsize1(2)]], 'nextplot', 'add', ...
                'ylim', [0 max(obj.MixedFP{end})/1000+1], 'xlim', [0.5 length(obj.MixedFP)*obj.NumSessions+0.5], 'yscale', 'linear', ...
            ...
            set(ha_HoldT_IQR,  'units', 'centimeters', 'position', [xlevel, ylevel3, plotsizeIQR], 'nextplot', 'add', ...
                'ylim', [0 max(obj.MixedFP{end})/1000+1], 'xlim', [0.5 obj.NumSessions+0.5], 'yscale', 'linear', ...
                'xtick', [1:3*obj.NumSessions], ...
                'xticklabels', obj.Dates, 'fontsize', 7);
            xlabel('Sessions')
            ylabel('IQR (s)')
            xline(WaitGap, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
            ...
            LateColor = [249, 74, 41]/255;
             ...
             for k = 1:length(obj.MixedFP)
                 ...
                 set(ha_pdf_progress(k), 'units', 'centimeters', 'position', [xlevel_k, ylevel2+0.5, plotsizePDF], 'nextplot', 'add', ...
                     'ylim', [0 MaxPDF*1.1],...
                     'xlim', [0 max(obj.MixedFP{end})+1000],'xtick',[0:500:max(obj.HoldTbinEdges)*1000], 'yscale', 'linear');
                 ...
                 line([obj.MixedFP{end} obj.MixedFP{end}], [0 MaxPDF*1.1], 'color', [0.5 0.5 0.5], 'linewidth', 1, 'linestyle', ':')
           ...
           legend([hp_early, hp_late], 'Early', 'Late', 'Box','off', 'Location','northeast')
            ...
            PressDurRange = [Lneg, max(obj.MixedFP{end})+2000];
            MarkerAlpha = 0.4;
            ...
            for i =1:obj.NumSessions
                ...
                line(ha(2), [StartTime StartTime], [-5 100], ...
                    'linestyle', ':' ,'color', [0.6 0.6 0.6],'linewidth', 1);
                if i == WaitGap+0.5
                    xline(ha(1),StartTime, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
                    xline(haRT,StartTime, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
                    xline(ha(2),StartTime, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);  
                ...
                for k =1:length(obj.MixedFP)
                    symbolSize = 5+10*(k-1);
                    ind_premature_presses = strcmp(iOutcome, 'Premature') & ismember(iFP,obj.MixedFP{k}) & iStage == 1;
                ...
                for k =1:length(obj.MixedFP)
                    symbolSize = 5+10*(k-1);
                    ind_late_presses = strcmp(iOutcome, 'Late') & ismember(iFP,obj.MixedFP{k}) & iStage == 1;
                    ...
                    ind_correct_presses = strcmp(iOutcome, 'Correct') & ismember(iFP,obj.MixedFP{k}) & iStage == 1;
            ...
            imagesc([1:obj.NumSessions*length(obj.MixedFP)], obj.HoldTbinEdges,  cell2mat(HoldTDistributionAll), [0 maxHoldT_PDF*1.2]);
            colormap(turbo)
            hbar = colorbar;
            xlevel = 11.5
            ...
            for k =1:length(obj.MixedFP)
                xline(WaitGap+obj.NumSessions*(k-1), '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
                if k ==1
                    te = ['<1000 ms'];%[num2str(obj.MixedFP(k)) ' ms']
                elseif k ==2
                    te = ['<1500 ms'];
                elseif k==3
                    te = ['=1500 ms'];
                end
                text(0+obj.NumSessions*(k-1)+1, max(obj.MixedFP{k})/1000+1-0.1, te, 'fontname', 'dejavu sans', 'fontsize', 8, 'color', 'w')
                if ~isempty(IndDCZ)
                    for j = 1:length(IndDCZ)
                        arrow([IndDCZ(j)+obj.NumSessions*(k-1), 0], [IndDCZ(j)+obj.NumSessions*(k-1) 0.15], ...
                            'color', ShadeCol, 'linewidth', 2, 'Length',3, 'TipAngle', 35);
                        pos = [IndDCZ(j)+obj.NumSessions*(k-1)-0.5, 0, 1, max(obj.MixedFP{end})/1000+1];
            ...
            xline(ha(3), WaitGap, '--', 'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
...
%             yyaxis left
            ...
            set(haRT2, 'Position', pos_org, 'xlim', [0 obj.NumSessions+1], 'xtick',[1:obj.NumSessions], ...
                ...
                'ylim', [0 maxRT*1000],'TickDir','in')
            
            yyaxis right
            set(gca,'YColor',[100,149,237]/255)
            colororder([100,149,237]/255)
            ylim([0.4 1])
            yticks([0.4:0.2:1])
            yticklabels({round(max(obj.NumTrialsPerSession).*[0.4:0.2:1])})
            ylabel('Trial Number','Position',[obj.NumSessions+0.5, 0.7])
            plot(1:obj.NumSessions,obj.NumTrialsPerSession./max( obj.NumTrialsPerSession),'^-','Color',[100,149,237]/255,'LineWidth',1,'MarkerSize',3)
            ...
            if length(unique(obj.WaitSessions))>1
                hui_2 = uicontrol('Style', 'text', 'parent', obj.Fig3, 'units', 'normalized', 'position', [0.3 0.965 0.4 0.03],...
                    'string', ['Protocol: ' obj.Protocols{1},'+',obj.Protocols{WaitGap+0.5}], 'fontweight', 'bold', ...
                    'backgroundcolor', [1 1 1], 'HorizontalAlignment', 'left' , 'fontname', 'dejavu sans');
            else
                hui_2 = uicontrol('Style', 'text', 'parent', obj.Fig3, 'units', 'normalized', 'position', [0.3 0.965 0.4 0.03],...
...
%             end;
        ...
        function Print(obj, targetDir)
            ...
            if ishghandle(obj.Fig5)
                savename = ['Fig5_ChemoPerformance_' upper(obj.Subject{1}) '_1_' num2str(1/obj.DoseSessions(strcmp(obj.Dates, obj.SessionChemo(1)))) 'DCZ'];
                if nargin==2
                    % check if targetDir exists
                    if ~contains(targetDir, '/') && ~contains(targetDir, '\')
                        % so it is a relative path
        ...
        function obj = PlotProbeTrial(obj, ind)

            set_matlab_default;
            col_perf = [85 225 0
                255 0 0
                140 140 140]/255;
            cProbe = [0,128,128]/255;
            FPColor = [189, 198, 184]/255;
            WhiskerColor = [132, 121, 225]/255;
            ShadeCol = [255, 212, 149]/255;
            colors = {[0 0 0.6], [255, 201, 60]/255};
            figure(213); clf(213)
            set(213, 'unit', 'centimeters', 'position',[2 2 18 18], 'paperpositionmode', 'auto', 'color', 'w')
            ...
            maxDur = 4000/1000; % for  plotting purpose
            maxRT = 2000/1000;
            

            ha(1) = axes;
            xlevel = [2 2+plotsize1(1)+1.5 2+plotsize1(2)+1.5 2+plotsize1(2)+1.5+plotsize1(2)+1.5];
            ylevel = [2 2+plotsize1(2)+1.5 2+plotsize1(2)+1.5+plotsize1(2)+1.5]-0.5;
            set(ha(1), 'units', 'centimeters', 'position', [xlevel(1), ylevel(3), plotsize1], 'nextplot', 'add', ...
                'ylim', [0 maxDur*1000], 'yscale', 'linear')
            xlabel('Time in session (sec)')
            ylabel('Press duration (msec)')
            ...
            set(ha(2), 'units', 'centimeters', 'position', [xlevel(1), ylevel(2), plotsize1], 'nextplot', 'add', ...
                'ylim', [0 maxDur*1000], 'yscale', 'linear')
            xlabel('Time in session (sec)')
            ylabel('Distribution (msec)')
% Press duration y range
            Lneg                   = -250; % for plotting response time
            PressDurRange = [0, max(obj.MixedFP)+2500];
            MarkerAlpha = 0.4;
            ...
            for i =1:obj.NumSessions
               ...
               HoldTDistributionAll = obj.PDF_HoldT_Sessions{i, 4}';
                ...
                if ~isempty(obj.TreatmentSessions)
                    if strcmp(obj.TreatmentSessions{i}, 'DCZ')
                        Treatment = 1;
                    elseif i~=obj.NumSessions && strcmp(obj.TreatmentSessions{i+1}, 'DCZ')
                        Treatment = -1;
                    end;
                end;
                ...
                iFP                                                =            obj.FP(iTrialsIndx);
                indPerformanceSliding                  =           find(obj.PerformanceSlidingWindow.Session == i);
                iCue                              = obj.Cue(obj.SessionIndex == i);

                % Check if this is a Chemo session

                set(ha(1), 'ylim', PressDurRange, 'xlim', [0 iReleaseTimes(end)+StartX1], 'yscale', 'linear');
                set(ha(2), 'ylim', PressDurRange, 'xlim', [0 0.5+StartX2], 'yscale', 'linear');
         
%                                 set(ha(2), 'ylim', [-5 100], 'xlim', [0 iReleaseTimes(end)+StartTime], 'yscale', 'linear', 'xtick', []);
%                 set(haRT, 'xlim', [0 iReleaseTimes(end)+StartTime])

                % draw shades
                ind_probe_presses = iCue == 0;
                if Treatment==1
                    axes(ha(1))
                    plotshaded([StartX1 StartX1 + iReleaseTimes(end)], [PressDurRange(1) PressDurRange(1); PressDurRange(2) PressDurRange(2)], ShadeCol, 0.5);
                    axes(ha(2))
                    plotshaded([StartX2 StartX2 + 0.5], [0 0; PressDurRange(2) PressDurRange(2)], ShadeCol, 0.5);
%                     length(iPressDurs(ind_probe_presses))
                    ProbeHoldTChemo = [ProbeHoldTChemo, iPressDurs(ind_probe_presses)];
                    ProbePressTChemo = [ProbePressTChemo, iPressTimes(ind_probe_presses)];
                elseif Treatment==-1
%                     length(iPressDurs(ind_probe_presses))
                    ProbeHoldTControl = [ProbeHoldTControl, iPressDurs(ind_probe_presses)];
                    ProbePressTControl = [ProbePressTControl, iPressTimes(ind_probe_presses)];
                end;

                % mark a new session
                line(ha(1), [StartX1 StartX1], [0 maxDur*1000], ...
                    'linestyle', ':' ,'color', [0.6 0.6 0.6],'linewidth', 1);
                line(ha(2), [StartX2 StartX2], [0 maxDur*1000], ...
                    'linestyle', ':' ,'color', [0.6 0.6 0.6],'linewidth', 1);

%                 % plot press times
%                 line(ha(1), [iPressTimes; iPressTimes]+StartX1, [Lneg; 0], 'color', 'b'); % all press times

                % Plot Probe Trial
               
                symbolSize = 15;
                
                scatter(ha(1),iPressTimes(ind_probe_presses)+StartX1, ...
                    1000*iPressDurs(ind_probe_presses), ...
                    25, cProbe, '+','Markerfacealpha', 0.8, 'linewidth', 1, 'SizeData', symbolSize, 'linewidth', 0.5);
             
                % Plot Probe HoldTime PDF
                plot(ha(2),obj.PDF_HoldT_Sessions{i, 4}+StartX2, obj.HoldTbinEdges*1000,...
                    'LineWidth', 1, 'Color',cProbe)
            
                StartX1 = iReleaseTimes(end) + StartX1+60;
                StartX2 = 0.5 + StartX2;
            end;
            ...
            set(ha_Press, 'units', 'centimeters', 'position', [xlevel(2), ylevel(3), plotsize_rt2], 'nextplot', 'add', ...
                'ylim', [0 maxDur*1000], 'yscale', 'linear','XLim',[0  maxDur*2000])
%             xlabel('Time in session (sec)')
            ylabel('Press duration (msec)')
            symbolSize = 15;
                
            scatter(ha_Press,ProbePressTControl, ...
                1000*ProbeHoldTControl, ...
                25, cProbe, '+','Markerfacealpha', 0.8, 'linewidth', 1, 'SizeData', symbolSize, 'linewidth', 0.5);
            line(ha_Press, [maxDur*1000 maxDur*1000], [0 maxDur*1000], ...
                    'linestyle', ':' ,'color', [0.6 0.6 0.6],'linewidth', 1);
            scatter(ha_Press,ProbePressTChemo+maxDur*1000, ...
                1000*ProbeHoldTChemo, ...
                25, cProbe, '+','Markerfacealpha', 0.8, 'linewidth', 1, 'SizeData', symbolSize, 'linewidth', 0.5);
            plotshaded([maxDur*1000 maxDur*2000], [PressDurRange(1) PressDurRange(1); PressDurRange(2) PressDurRange(2)], ShadeCol, 0.5);

            

            ha_DisT = axes;

            set(ha_DisT, 'units', 'centimeters', 'position', [xlevel(2), ylevel(2), plotsize_rt2], 'nextplot', 'add', ...
                'ylim', [0 maxDur*1000], 'yscale', 'linear','XLim',[0,1])
            xlabel('PDF(1/msec)'); xticklabels({'0', '0.5  0', '0.5'})
            ylabel('Distribution (msec)')
            plot(ha_DisT,obj.PDF_HoldT_Control(:,4), obj.HoldTbinEdges*1000,...
                    'LineWidth', 1, 'Color',cProbe)
            line(ha_Press, [0.5 0.5], [0 maxDur*1000], ...
                    'linestyle', ':' ,'color', [0.6 0.6 0.6],'linewidth', 1);
            plot(ha_DisT,obj.PDF_HoldT_Chemo(:,4)+0.5, obj.HoldTbinEdges*1000,...
                    'LineWidth', 1, 'Color',cProbe)
            plotshaded([0.5 1], [PressDurRange(1) PressDurRange(1); PressDurRange(2) PressDurRange(2)], ShadeCol, 0.5);

            haCDF = axes;

            set(haCDF, 'units', 'centimeters', 'position', [xlevel(4), ylevel(1), plotsize_rt2], 'nextplot', 'add', ...
                'ylim', [0 1], 'yscale', 'linear', 'xlim', [0 max(obj.HoldTbinEdges*1000)])
            xlabel('Press duration (msec)')
            ylabel('CDF')
            
            plot(haCDF, obj.HoldTbinEdges*1000,obj.CDF_HoldT_Control(:,4),...
                'LineWidth', 1, 'Color',colors{1})
            
            plot(haCDF, obj.HoldTbinEdges*1000,obj.CDF_HoldT_Chemo(:,4),...
                'LineWidth', 1, 'Color',colors{2})
%             if length(obj.SessionChemo)>2
            try
                plotshaded(obj.HoldTbinEdges*1000,obj.CDF_HoldT_MedianCI95_Control{4}, colors{1}, 0.5)
                plotshaded(obj.HoldTbinEdges*1000,obj.CDF_HoldT_MedianCI95_Chemo{4}, colors{2}, 0.5)
            end
%             end
            haPDF = axes;

            set(haPDF, 'units', 'centimeters', 'position', [xlevel(3), ylevel(1), plotsize_rt2], 'nextplot', 'add', ...
                 'yscale', 'linear', 'xlim', [0 max(obj.HoldTbinEdges*1000)])
            xlabel('Press duration (msec)')
            ylabel('PDF(1/msec)')
            plot(haPDF, obj.HoldTbinEdges*1000,obj.PDF_HoldT_Control(:,4),...
                'LineWidth', 1, 'Color',colors{1})
                        plot(haPDF, obj.HoldTbinEdges*1000,obj.PDF_HoldT_Chemo(:,4),...
                'LineWidth', 1, 'Color',colors{2})
%             if length(obj.SessionChemo)>2
              try
                plotshaded(obj.HoldTbinEdges*1000,obj.PDF_HoldT_MedianCI95_Control{4}, colors{1}, 0.5)
                plotshaded(obj.HoldTbinEdges*1000,obj.PDF_HoldT_MedianCI95_Chemo{4}, colors{2}, 0.5)
              end
            
           haViolin = axes;

           set(haViolin, 'units', 'centimeters', 'position', [xlevel(1), ylevel(1), plotsize_rt2], 'nextplot', 'add', ...
                'ylim', [0 max(obj.HoldTbinEdges*1000)], 'yscale', 'linear' )%,'xlim', [0 3]
           
           ylabel('Press duration (msec)')
           hVio1 = violinplot([ProbeHoldTControl*1000 ProbeHoldTChemo*1000],  [ones(1,length(ProbeHoldTControl))  2*ones(1,length(ProbeHoldTChemo))]);%, 'Width', 0.4, 'ViolinAlpha', 0.2, 'ShowWhiskers', false,'Bandwidth', 0.2
           
           for iv =1:length(hVio1)
                hVio1(iv).ScatterPlot.SizeData                    = 12;
                hVio1(iv).ScatterPlot.MarkerFaceColor      = colors{iv};
                hVio1(iv).ScatterPlot.MarkerFaceAlpha      = 0.5;
                hVio1(iv).ViolinPlot.FaceColor                     = [.8 .8 .8];
%                hVio1(iv).EdgeColor = [0 0 0];
%                hVio1(iv).WhiskerPlot.Color = WhiskerColor;
%                hVio1(iv).WhiskerPlot.LineWidth = 1.5;
%                hVio1(iv).ScatterPlot.MarkerFaceColor = 'k';
%                hVio1(iv).ScatterPlot.SizeData = 10;
%                hVio1(iv).Vifigure(); olinPlot.FaceColor = [0.8 0.8 0.8];
%                hVio1(iv).BoxColor=[253, 138, 138]/255;
%                hVio1(iv).BoxWidth = 0.03;
           end;
           xticklabels({'Saline','DCZ'})
           sgtitle( [cell2mat(obj.Subject),'   ', cell2mat(obj.Protocols(1)), '   ProbeTrialAnaylsis   '], 'fontsize',10)
           FigureName = ['Fig7_',cell2mat(obj.Subject),'_', cell2mat(obj.Protocols(1)), '_ProbeTrialAnaylsis'];
           FigureName = fullfile(pwd, FigureName);
            print (gcf,'-dpng', FigureName)
            print (gcf,'-djpeg', FigureName)
            saveas(gcf, FigureName, 'fig')
        end
        function obj = PlotNumTrial(obj, ind)

            plotsize1 = [4 5];
            xlevel = [1.5 3+4];
            ylevel = 1.5;
            DoseKind = unique(obj.DoseSessions);
            Fig33 = figure(33);
            set(Fig33,'unit', 'centimeters', 'position',[2 2 18 5.5*(length(DoseKind)-1)+2], 'paperpositionmode', 'auto', 'color', 'w')
            for i = 2: length(DoseKind)
            ControlNum = obj.NumTrialsPerSession(find(obj.DoseSessions==DoseKind(i))-1);
            ChemoNum = obj.NumTrialsPerSession(find(obj.DoseSessions==DoseKind(i)));
            
            ControlCorrNum = obj.NumCorrectTrialsPerSession(find(obj.DoseSessions==DoseKind(i))-1);
            ChemoCorrNum = obj.NumCorrectTrialsPerSession(find(obj.DoseSessions==DoseKind(i)));
            try 
            PostNum = obj.NumTrialsPerSession(find(obj.DoseSessions==DoseKind(i))+1);
            PostCorrNum = obj.NumCorrectTrialsPerSession(find(obj.DoseSessions==DoseKind(i))+1);    
            end
            
            ha(1) = axes;
            set(ha(1), 'units', 'centimeters', 'position', [xlevel(1), ylevel, plotsize1], 'nextplot', 'add', ...
                'yscale', 'linear')%, xlim, [0,4], xticks,[0:4],xticklabels, {'','Saline', 'Chemo', 'PostDay',''}

            ylabel('Trial Number')            
            X = categorical({'Saline', 'Chemo', 'PostDay'});
            X = reordercats(X,{'Saline', 'Chemo', 'PostDay'});
           
            bar(ha(1),X, [mean(ControlNum) mean(ChemoNum) mean(PostNum)])
            hold on;
            group = [repelem(1,length(ControlNum)) repelem(2,length(ChemoNum)) repelem(3,length(PostNum))];
            scatter( group,[ControlNum ChemoNum PostNum])
            p1_anova(i) = anova1([ControlNum ChemoNum PostNum],group,'off');
            if p1_anova(i)<0.05 
                [h1,p1] = ttest(ControlNum,ChemoNum);
                [h2,p2] = ttest(ChemoNum, PostNum);
                [h3,p3] = ttest2(ControlNum,PostNum);
            end
            SigDiffPlot(ha(1),1, 2, p1)  
            SigDiffPlot(ha(1),2, 3, p2) 
            ha(2) = axes;
                set(ha(2), 'units', 'centimeters', 'position', [xlevel(2), ylevel, plotsize1], 'nextplot', 'add', ...
                 'yscale', 'linear')
            ylabel('Correct Trial Number')

            bar(X, [mean(ControlCorrNum) mean(ChemoCorrNum) mean(PostCorrNum)])   
%             groupCorr = [repelem(1,length(ControlCorrNum)) repelem(2,length(ChemoCorrNum)) repelem(3,length(PostCorrNum))];
            scatter( group, [ControlCorrNum ChemoCorrNum PostCorrNum])
            p2_anova(i) = anova1([ControlCorrNum ChemoCorrNum PostCorrNum], group,'off');
            if p2_anova(i)<0.05 
                [h1,p1] = ttest(ControlCorrNum,ChemoCorrNum);
                [h2,p2] = ttest(ChemoCorrNum, PostCorrNum);
                [h3,p3] = ttest2(ControlCorrNum,PostCorrNum);
            end
            SigDiffPlot(ha(2),1, 2, p1)  
            SigDiffPlot(ha(2),2, 3, p2) 
            ylevel = ylevel+6;                       
            end
        end

        function value = get.AnticipationChemo(obj)
            if find(obj.IndexSessionsChemo==1)
                AtcChemo =  obj.Anticipation(:,find(obj.IndexSessionsChemo==1))-obj.Anticipation(:,find(obj.IndexSessionsControl==1));
                tBin = obj.AnticipationBin(end)-obj.AnticipationBin(end-1);
                value = AtcChemo.*tBin.*100;
                value = value(all(~isnan(value),2),:);

                flag = 1;
                if flag == 1
                    figure;
                    counts = find(obj.IndexSessionsChemo==1);
                    counts = 1:length(counts);
                    line([obj.AnticipationBin(1) obj.AnticipationBin(end)],[0 0],'linestyle', '--','Color','k','HandleVisibility','off')

                    hold on;
                    plot(obj.AnticipationBin(1:end-1), value);
                    %                     legend({num2str(counts')})
                    hold on;
                    
                    AtcMean = mean(value', 'omitnan');
                   
                    %                 AtcCI = [AtcCI, nan(2,1)];

                    plot(obj.AnticipationBin(1:end-1),AtcMean,'LineWidth', 2, 'Color','k');
                    %                 legend({'mean'})
                    try
                    AtcCI = bootci(1000,@mean,value');
                    plotshaded(obj.AnticipationBin(1:end-1)',AtcCI, [255, 201, 60]/255, 0.5);
                    legend([strsplit(num2str(counts)),'mean','bootci'])
                    end
                    hold off;
                                        xlabel('Time(s)')
                    ylabel('chemo-control Anticipation(%)')
%                     ymax = max(0.2,AtcCI);
                    
                    sgtitle( [cell2mat(obj.Subject),'   ', cell2mat(obj.Protocols(1)), '   Anticipation   '], 'fontsize',10)
                    FigureName = ['Fig8_',cell2mat(obj.Subject),'_', cell2mat(obj.Protocols(1)), '_Anticipation'];
                    FigureName = fullfile(pwd, FigureName);
                    print (gcf,'-dpng', FigureName)
                    print (gcf,'-djpeg', FigureName)
                    saveas(gcf, FigureName, 'fig')
