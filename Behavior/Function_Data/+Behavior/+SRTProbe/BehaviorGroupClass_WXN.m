classdef BehaviorGroupClass_WXN
...
%         Cue % is always 1 for SRRT (just cut it out then)
    ...
    methods
        function obj = BehaviorGroupClass_WXN(SRTClassAll)
            ...
            obj.NumCorrectTrialsPerSession           =                 cellfun(@(x)x.Performance.N_press(4).*x.Performance.CorrectRatio(4).*0.01, SRTClassAll);
...
%         end;
        ...
        function value = get.HoldT_Control(obj)
            ...
            for k = 1:length(obj.MixedFP)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsControl)) & obj.Stage == 1 & (~strcmp(obj.Outcome, 'Dark')') & obj.FP == obj.MixedFP(k) & (~strcmp(obj.Outcome, 'NAN')');
        ...
        function value = get.HoldT_Chemo(obj)
            HoldT                               =        cell(1, length(obj.MixedFP)+1);
            for k = 1:length(obj.MixedFP)
                IndexControlTrials          =       ismember(obj.SessionIndex, find(obj.IndexSessionsChemo)) & obj.Stage == 1 & (~strcmp(obj.Outcome, 'Dark')') & obj.FP == obj.MixedFP(k) & (~strcmp(obj.Outcome, 'NAN')');
                HoldT{k}                           =       obj.HoldTime(IndexControlTrials);
            end;
            if find(obj.Cue==0)
...
%         end;
        ...
        function obj = PlotChemoEffect(obj, ind)
            ...
            xlevel = xlevel +6.5;
            maxIQR = max([obj.IQR_HoldT_Control(1:length(obj.MixedFP)) obj.IQR_HoldT_Chemo(1:length(obj.MixedFP))])*1.25;
...
% %         wwww   function PlotPrePostLesion(obj, varargin)
%             set_matlab_default;
%             % This is to plot pre and post lesion hold time PDF, which gives
%             % a quick show on the effect
%             ToPlot = 'Sessions';
...
%             end;
        ...
        function Print(obj, targetDir)
            ...
            if ishghandle(obj.Fig5)
                savename = ['Fig5_ChemoPerformance_' upper(obj.Subject{1}) '_1_' num2str(1/obj.DoseSessions(find(strcmp(obj.Dates, obj.SessionChemo(1)),1))) 'DCZ'];
                if nargin==2
                    % check if targetDir exists
                    if ~contains(targetDir, '/') && ~contains(targetDir, '\')
                        % so it is a relative path
