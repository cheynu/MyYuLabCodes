...
% plotSessionLeverPressVI generates a session-level summary figure for the
% LeverPress-VI task.
%
% Input:
%   vb              - output structure generated by track_training()
%   opts.SavePath   - folder used to save the figure
%
% Output:
%   A multi-panel PNG figure summarizing within-session behavioral dynamics.
%
% Figure structure:
%
% 1) Event rate across session
%   - Press, BadPokeFirst, and GoodPokeFirst are counted in 5-min bins.
%   - This panel provides a coarse view of response rate and poke strategy
%     over the session.
%
% 2) Press-bout structure by press rank
%   - Presses are grouped by their rank within a press bout.
%   - The upper row shows press rate as a function of press rank.
%   - The lower row shows:
%       N        = number of presses at each rank
%       P(Stay)  = probability that the next event is another press
%   - Separate panels are shown for:
%       All presses
%       Presses after GoodPoke
%       Presses after BadPoke
%
% 3) Distribution of key behavioral intervals
%   - Cumulative distributions are plotted on a log-scaled x-axis for:
%       MT             = release-to-poke movement time
%       Restart        = port-exit-to-next-press latency
%       Inter-press    = interval between consecutive presses
%       Inter-badpoke  = interval between BadPokeFirst events
%       Inter-reward   = interval between rewarded GoodPokeFirst events
%       VI             = experienced VI duration
%
% 4) Movement time distribution
%   - Probability density of movement time is compared between:
%       BadPoke
%       GoodPoke
%   - Movement time is defined as the interval from lever release to the
%     first poke-in of the following poke sequence.
%
% 5) Restart latency distribution
%   - Probability density of restart latency is compared between:
%       BadPoke
%       GoodPoke
%   - Restart latency is defined as the interval from port exit to the next
%     lever press.
%
% 6) Full session progress plot
%   - Press rate is plotted across session time in 1-min bins.
%   - The red trace shows a smoothed press-rate estimate.
%   - The horizontal black line indicates mean press rate.
%   - Gray shaded regions indicate VI periods.
%   - Event rasters are overlaid at the bottom:
%       black ticks = BadPress / BadPoke
%       green ticks = GoodPress
%       blue ticks  = GoodPoke
%       red ticks   = OmitGoodPoke
%   - The right y-axis shows the stay probability,
%     P(next event after press is another press).
%
% In brief, this function visualizes:
%   1) response vigor across the session;
%   2) press-bout organization;
%   3) switching between pressing and poking;
%   4) timing distributions of movement, restart, press, reward, and VI;
%   5) differences between GoodPoke- and BadPoke-related behavior.
...
t_GoodPress = vb.Responses.Time(contains(vb.Responses.Type,'GoodPress'));
t_BadPress = vb.Responses.Time(contains(vb.Responses.Type,'BadPress'));
t_GoodPokeFirst = vb.Responses.Time(contains(vb.Responses.Type,'GoodPokeFirst'));
t_RewardPokeFirst = vb.Responses.Time(contains(vb.Responses.Type,'RewardGoodPokeFirst'));
t_OmitPokeFirst = vb.Responses.Time(contains(vb.Responses.Type,'OmitGoodPokeFirst'));
t_BadPokeFirst = vb.Responses.Time(contains(vb.Responses.Type,'BadPokeFirst'));
...
iwi = diff(t_RewardPokeFirst); % inter-water (reinforcer) interval
...
nEpoch = floor(0.2+vb.SessionDuration/stepEpoch); % 30 seconds gap is allowed
edgesEpoch = 0:stepEpoch:nEpoch*stepEpoch;
ticksEpoch = 1:nEpoch;
...
Bout.AllPress.IRI = cell(size(boutTicks));
...
Bout.Stay.Good = [];
Bout.Stay.Bad = [];
Bout.MeanLength.All = mean(vb.PressBoutLength,'omitnan');
for i=1:length(boutTicks)
    ...
    Bout.AllPress.IRI{i} = thisIRI; % IRI (second)
    Bout.AllPress.isAfterGood{i} = thisType;
    Bout.AllPress.isStay{i} = thisStay;
    % all
    idxFilter = true(size(Bout.AllPress.isAfterGood{i}));
    thisdata = Bout.AllPress.IRI{i}(idxFilter);
    mean_rate = 60./mean(thisdata,'omitnan');
    if length(thisdata)>1
        ci_rate = 60./bootci(nBoot, @mean, thisdata);
    else
        ci_rate = [NaN;NaN];
    end
...
if ~isempty(t_OmitPokeFirst)
    line(repmat(t_OmitPokeFirst,[1 2]),[0 pokeLineLen*2]+pokeLineBottom, 'color', cRed, 'linewidth', 0.6, 'LineStyle', '-', 'Marker', 'none');
end
...
ylim(N_Lim);
N_Ticks = yticks;
line(repmat(Bout.MeanLength.All,[1 2]), N_Lim, 'color', 'k', 'linestyle', ':', 'LineWidth', 1);
