...
info = obj.TrialInfo;
sort_refs = {info.Cued, info.PortCorrect};
sort_code = {0, obj.LeftRight};

time_sorted = obj.sort_data(obj.TimeWarpHD, sort_refs, sort_code)';
...
color_cell = {{GPSColor.PortL; GPSColor.PortR}};
lw_cell = repmat({{lw}}, 2, 1);
ls_cell = repmat({{ls}}, 2, 1);

%%
fig = figure(11); clf(fig);
set(fig, 'Visible', 'on', 'Units', 'centimeters', 'Position', [5 5 16 4.2], 'Color', 'w', 'toolbar', 'none');

fig_title = sprintf("%s / %s / %s / %s", obj.Subject, obj.Session, obj.Protocol, obj.Label);
set_fig_title(fig, fig_title);

% angle head
ax_ang   = plt.assign_ax_to_fig(fig, 1, 1, [1.5 1 4 2], ax_sz);
...
cellfun(@(x) set(x, 'XLim', x_lim, 'YLim', [y_lim_ang_1 y_lim_ang_2], 'XTick', [0 1], 'XTickLabel', ["In", "Out"]), ax_ang);
cellfun(@(x) plt.add_shade(x, FP_zone, 'alpha', .1), ax_ang);

xlabel(ax_ang{1}, "Time warped", 'FontWeight', 'bold');
ylabel(ax_ang{1}, "Head angle (°)", 'FontWeight', 'bold');

% pos-x head
ax_posx   = plt.assign_ax_to_fig(fig, 1, 1, [6.5 1 4 2], ax_sz);
...
cellfun(@(x) set(x, 'XLim', x_lim, 'YLim', [y_lim_posx_1 y_lim_posx_2], 'XTick', [0 1], 'XTickLabel', ["In", "Out"]), ax_posx);
cellfun(@(x) plt.add_shade(x, FP_zone, 'alpha', .1), ax_posx);

ylabel(ax_posx{1}, "Head pos-X (pix)", 'FontWeight', 'bold');

% pos-y head
ax_posy   = plt.assign_ax_to_fig(fig, 1, 1, [11.5 1 4 2], ax_sz);
...
cellfun(@(x) set(x, 'XLim', x_lim, 'YLim', [y_lim_posy_1 y_lim_posy_2], 'XTick', [0 1], 'XTickLabel', ["In", "Out"]), ax_posy);
cellfun(@(x) plt.add_shade(x, FP_zone, 'alpha', .1), ax_posy);

ylabel(ax_posy{1}, "Head pos-Y (pix)", 'FontWeight', 'bold');
