...
function plot_perf_progress(obj, ax, perf, ls, lw, ms, lag)
if nargin<7
    lag = 0;
end
x = (1:obj.NumSessions) + lag;
for i = 1:length(obj.PerformanceType)
    perf_this = obj.PerformanceType(i);
    plot(ax, x, 100*perf.(perf_this), ...
        'LineStyle', ls, 'LineWidth', lw, 'Color', GPSColor.(perf_this), ...
        'Marker', 'o', 'MarkerSize', ms, 'MarkerFaceColor', GPSColor.(perf_this), 'MarkerEdgeColor', 'w');
end
