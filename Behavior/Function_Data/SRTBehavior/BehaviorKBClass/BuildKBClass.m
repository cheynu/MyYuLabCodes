function BClassGroup = BuildKBClass(Protocol, options)
...
if isempty(options.Subjects)
    Subjects = arrayfun(@(x) string(x.name), ...
        getSubContents(fullfile(pwd,'2_ANMs')), 'UniformOutput', true);
else
...
ptclBClass = cell(nSubject, 1);  % rxiv cell for building BClass_Group
for i = 1:nSubject  % i = subject num

    if exist(ptclPath{i}, 'dir') == 7  % if folder "./Subject/Protocol" exist
        ...
        iLog = TrainingLog(string(TrainingLog.Subject) == Subjects(i), :);
        ...
        for j = 1:length(dataPath)  % j = date num
            ...
            if options.reExtract || length(dir('BClass*.mat')) ~= 1
                ...
                FileNames = arrayfun(@(x)x.name, dir(Subjects(i)+"*Kornblum*.mat"), 'UniformOutput', false);
                for k = 1:length(FileNames)  % k = session num
                    if k == 1
                        BClass = BehaviorKB(FileNames{k});
                    else
                        kBClass = BehaviorKB(FileNames{k});
                        BClass = BClass.merge(kBClass);  % Merge files of one day
                    end
                    jGroup = jLog.Group;
                    if iscell(jGroup)
                        BClass.Group = string(jLog.Group{:});
                    elseif isnan(jGroup)
                        BClass.Group = "";
                    else
                    ...
                    if iscell(jExp)
                        BClass.Experiment = string(jLog.Experiment{:});
                    elseif isnan(jExp)
                        BClass.Experiment = "";
                    else
            ...
            if options.plotProgress
                prgFig = BClass.plot();
                BClass.print("Figure", prgFig, "savePath", "Fig");
        ...
        if options.reExtract || length(dir('BClassIndiv*.mat')) ~= 1
            ...
            BClassIndiv = BehaviorKB_Indiv(sbjBClass);
            BClassIndiv.save();
            BClassIndiv.save("savePath", fullfile(indivPath, Subjects(i)));
        else
        ...
        if options.plotIndividual
            ...
            BClassIndiv.print("Figure", indivFig, "savePath", fullfile(summaryPath, 'Fig'));
            BClassIndiv.print("Figure", indivFig, "savePath", fullfile(indivPath, '0_Fig'));
        end
...
if options.reExtract || length(dir('BClassGroup*.mat')) ~= 1
    ...
    BClassGroup = BehaviorKB_Group(ptclBClass);
    BClassGroup.save();
else
