function out = screen_top_tracking(T)
...
%   out.trials_question  cell array of suspicious trial IDs
    ...
    % basic checks
    %-----------------------------
    required_vars = {'time','head_x','head_y','trial'};
    ...
    if isempty(Tb_click)
        warning('screen_tracking:NoValidXY', ...
            'No valid head_x/head_y rows found for body part: %s', this_body);
        return;
    end
    ...
    hf1 = figure('Name', sprintf('screen tracking: %s', this_body), ...
                 ...
                 'Position',[2 2 25 8]);
    ax = subplot(1, 3, 1);
    scatter(xg, yg, 4, 'filled');
    xlabel('x_s');
    ylabel('head_y');
    title(sprintf('%s: head_x vs head_y', this_body), 'Interpreter', 'none');
    axis equal;
    ...
    figure(hf1);
    title({
        sprintf('%s: head_x vs head_y', this_body), ...
    ...
    %-----------------------------
    % plot head_y versus time in one axes
    %-----------------------------
    ...
    xlabel(ax, 'time');
    ylabel(ax, 'head_y');
    title(ax, sprintf('%s | suspicious trials | head_y vs time', this_body), ...
        'Interpreter', 'none');
