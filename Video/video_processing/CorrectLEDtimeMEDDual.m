function tLEDnew = CorrectLEDtimeMEDDual(TriggerTimeMED, FrameInfo)
...
tframeR            =     FrameInfo.tFrameMED; % frame timestamps in b's time domain

IndTrigger        =     find(tbeh_trigger>tframeR(1) & tbeh_trigger< tframeR(end));
...
for i=1:length(tTrigger)
    ...
    ind_LEDon = find(tframeR>=itrigger, 1, 'first');
