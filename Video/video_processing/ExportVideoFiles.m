            ...
            if ~isempty(perf)
                ...
                if isempty(check_this_file)  || remake % only make new video, unless told to remake all files.
                    ...
                    for ifile = 1:FileNum
                        ...
                        VidFrameIdx_thisfile = frameinfo.AviFrameIndx(VidFrameIdx(IdxFrames_thisfile));  % these are the frame index in this video
                    ...
                    for k =1:size(img_extracted, 3)
                        ...
                        hf25 = figure(25); clf
                        set(hf25, 'name', 'side view', 'units', 'centimeters', 'position', [ 3 5 15*xmax/ymax 18], 'PaperPositionMode', 'auto', 'color', 'w')
                        
                        ha= axes;
                        set(ha, 'units', 'centimeters', 'position', [0 3 15*xmax/ymax 15], 'nextplot', 'add', 'xlim',[0 xmax], 'ylim', [0 ymax], 'ydir','reverse')
                        axis off
                        % plot this frame:
