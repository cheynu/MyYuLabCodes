function ExportVideoClipFromAvi(BehTable, FrameTable, varargin)
    ...
    for i=1:2:size(varargin,2)
        switch varargin{i}
            case {'Event'}
                event = varargin{i+1};
            case {'Pre'}
                tPre = varargin{i+1}; % pre- and post-event periods.
            case {'Post'}
                tPost = varargin{i+1}; % pre- and post-event periods.
            case {'ANM'}
                anm = varargin{i+1}; % animal name
            case {'BehaviorType'}
                beh_type = varargin{i+1}; % animal name
            case {'Session'}
...
for i =1:length(tBehEvent) % i is also the trial number
    ...
    IndThisClip                     =           find(FrameTable.tFrames2Bpodms>=itEvent - tPre & FrameTable.tFrames2Bpodms<= itEvent + tPost);
    [~, IndThisFrame]         =            min(abs(FrameTable.tFrames2Bpodms - itEvent));
    ...
    iFrameTimesBpod = FrameTable.tFrames2Bpodms(IndThisClip);
    ...
    VidMeta.FrameTimesB     =          FrameTable.tFrames2Bpodms(IndThisClip);                        % frame time in ms in behavior time
