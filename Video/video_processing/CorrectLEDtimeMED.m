...
tbeh_trigger    =     TriggerTimeMED;  % in ms
tframeMED            =     FrameInfo.tFrameMED; % frame timestamps in b's time domain
IndTrigger        =     find(tbeh_trigger>tframeMED(1) & tbeh_trigger< tframeMED(end));
...
for i=1:length(tTrigger)
    ...
    ind_LEDon = find(tframeMED>=itrigger, 1, 'first');
...
% remove empty ones
ind_toremove = find(sum(roi_collect, 2)==0);

TriggerTimeMED(ind_toremove) = [];
tLEDonOrg(ind_toremove) = [];
roi_collect(ind_toremove, :) = [];
tTrigger(ind_toremove) = [];
tbeh_trigger(ind_toremove) = [];
...
for i =1:size(roi_collect, 1)
    ...
    if ~isempty(ind_rising) && length(alltframes_extracted)>ind_rising-1
