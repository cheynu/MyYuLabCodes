function tLEDnew = CorrectLEDtimeEphysTriggerTime(FrameInfo)
% revised from CorrectLEDtimeMED
% Jianing Yu
% 4/29/2021
% 5/5/2022 revised
% 12/4/2022 revised for MED
% 1/2/2025 revised for trigger time
% use FrameInfo.ROI to get the ROI out 
 
tbeh_trigger               =     FrameInfo.TriggerTimeEphys_ms;  % in ms
tframeEphys            =     FrameInfo.tFramesMapped2Ephys_ms; % frame timestamps in Ephys' world
% find triggers that are included in this video segments. 
tTrigger                      =     tbeh_trigger(tbeh_trigger>tframeEphys(1) & tbeh_trigger< tframeEphys(end));
tframeVid                   =     FrameInfo.tframeOrg;
...
for i=1:length(tTrigger)
    ...
    ind_LEDon = find(tframeEphys>=itrigger, 1, 'first');
