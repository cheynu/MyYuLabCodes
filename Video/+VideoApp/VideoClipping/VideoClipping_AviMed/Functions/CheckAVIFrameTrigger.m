...
hf19 = figure(19); clf(hf19,'reset'),
set(hf19, 'name', 'Check press', 'units', 'centimeters', 'position', [5 5 40 20],...
    'paperpositionmode', 'auto','renderer','opengl','toolbar','none','resize','off');
...
%         ha_axes(i, 1) = axes('units', 'centimeters', 'position', [1+(j-1)*4 1+(i-1)*4.5 4 4], 'nextplot', 'add', 'xlim', [0 1000], 'ylim', [0 1000], 'ydir', 'reverse');
        axes('units', 'centimeters', 'position', [1+(j-1)*4 1+(i-1)*4.5 4 4], 'nextplot', 'add', 'xlim', [0 1000], 'ylim', [0 1000], 'ydir', 'reverse');
        imagesc(this_frame, [0 255]);
...
hf20 = figure(20); clf(hf20,'reset'),
set(hf20, 'name', 'Check press', 'units', 'centimeters', 'position', [5 5 10 10], 'paperpositionmode', 'auto');
...
print(hf20,'-dpng', ['CheckTriggerFrames']);
