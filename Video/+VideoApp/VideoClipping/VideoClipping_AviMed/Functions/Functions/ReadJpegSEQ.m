...
% Last modified 03.30.2022 by Yu Chen (AllocatedFrames ulong)
...
fseek(fid,572,'bof');
headerInfo.AllocatedFrames = fread(fid,1,'ulong',endianType)-1;
fseek(fid,620,'bof');
...
% fclose(tempFile);
% I{1,1} = imread(tempName);
I{1,1} = uint8(py.cv2.imdecode(py.numpy.uint8(py.numpy.array(JpegSEQ)),uint8(0)));
