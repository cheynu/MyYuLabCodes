function CheckSeqFramePressRelease_Bpod(bt, FrameInfo)
...
tPress              =     bt.TimeElapsed(bt.Outcome=="Cor").*1000; % only use correct presses
tRelease          =     (bt.TimeElapsed(bt.Outcome=="Cor") + bt.HT(bt.Outcome=="Cor")).*1000;
tframeb            =     FrameInfo.tFramesInB;
IndPress          =     find(tPress> tframeb(1)+2000 & tRelease<tframeb(end)-2000);
if isempty(IndPress)
    tPress              =     bt.TimeElapsed(bt.Outcome~="Pre").*1000; % avoid error
    tRelease          =     (bt.TimeElapsed(bt.Outcome~="Pre") + bt.HT(bt.Outcome~="Pre")).*1000;
    IndPress          =     find(tPress> tframeb(1)+2000 & tRelease<tframeb(end)-2000);
end
tPress              =     tPress(IndPress);
tRelease          =     tRelease(IndPress);
IndSelected     =     randperm(length(IndPress), min(4,length(IndPress)));
