function tLEDnew = CorrectLEDtime_Bpod(bt, FrameInfo)
...
tbeh_trigger            = 1000.* (bt.TimeElapsed(bt.Outcome~="Pre") + bt.FP(bt.Outcome~="Pre"));  % in ms
tframeb                 = FrameInfo.tFramesInB; % frame timestamps in b's time domain
IndTrigger              = tbeh_trigger>tframeb(1) & tbeh_trigger< tframeb(end);
