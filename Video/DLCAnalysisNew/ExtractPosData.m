...
figure; 
set(gcf, 'unit', 'centimeters', 'position',[2 2 18 18], 'paperpositionmode', 'auto', 'visible', 'on')
plot(XYPos(:, 1), XYPos(:, 2), 'k+', 'linewidth', 1);

set(gca,'unit', 'normalized', 'nextplot', 'add', 'ydir', 'reverse',...
    'xlim', [0 400], 'ylim', [0 400],  'position',[0.1 0.1 0.8 0.8])
hold on
...
clc;
Nclus = [];
while isempty(Nclus) ||~isnumeric(Nclus) || Nclus >3 || Nclus<1 
    Nclus = input('Enter the number of clusters[1-3]: '); 
end;
...
for i=1:Nclus
    ...
    while ~imhappy
        input(sprintf('Draw the perimeter of cluster #%2.0d | Hit "Enter" when ready', i));
...
save(pos_name, 'PosData')

% Make an Excel sheet. 
MakeSpreadSheetDLCPos(PosOut)