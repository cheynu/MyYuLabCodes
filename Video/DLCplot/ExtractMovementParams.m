function PressOut = ExtractMovementParams(file,savepath)
switch nargin
    case 1
        savepath = pwd;
end
...
[~,~,raw] = xlsread(fullfile(filepath,filename)); % readtable would be better

inddash =  strfind(filename, '_');

Press.RatName     = filename(1:inddash(1)-1);
Press.SessionName = raw{2, 2};
Press.PressIndex  = cell2mat(raw(2:end, 3)); % risk of the order changed
...
tosavename = fullfile(savepath,...
    ['ManualTracking' '_' num2str(PressOut.SessionName)]);
[~,~] = mkdir(savepath);
