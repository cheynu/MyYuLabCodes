function PlotManualTrackingPressOut(PressOut, PrefPaw, IndPost, IndPreLesion, IndPostLesion, savepath)
switch nargin
    case 5
        savepath = pwd;
end
...
%% Plot sessions
hf = figure(26); clf
set(hf, 'unit', 'centimeters', 'position',[2 2 14 18], 'paperpositionmode', 'auto','renderer','Painters' )
...
for i = 1:length(PressOut)
    tRelease= PressOut(i).ReleaseOnset(~isnan(PressOut(i).ReleaseOnset) & PressOut(i).PressPaw==1 & PressOut(i).Outcome>=0);
...
text(1, 8, sprintf('p=%2.5g', P_Release), 'fontsize', 8)
text(1, 5, sprintf('Pre: %2.0f ms', tReleasePre_mean), 'fontsize', 8)
text(1, 2, sprintf('Post: %2.0f ms', tReleasePost_mean), 'fontsize', 8)
...
tosavename = fullfile(savepath,...
    ['ManualTrackingPeriLesion' '_' PressOut(1).RatName '_Paw_' Paw]);
