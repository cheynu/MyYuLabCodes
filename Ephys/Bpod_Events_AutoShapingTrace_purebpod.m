function events = Bpod_Events_AutoShapingTrace_purebpod(sd);
% 2/20/2021
...
for k =1:Ntrials
    ...
    events.Triggers = [events.Triggers; t_trial+sd.RawEvents.Trial{k}.States.PlayTone(1)];
    ...
    if ~isempty(sd.Custom.PokeTime_Reward{k})  % failed attempt
        events.GoodPokeIn = [events.GoodPokeIn t_trial+sd.RawEvents.Trial{k}.States.RewardDelivery(1)];
        events.Reward = [events.Reward t_trial+[sd.RawEvents.Trial{k}.States.RewardDelivery(1); sd.RawEvents.Trial{k}.States.RewardDelivery_4(2)]];
        events.GoodWait = [events.GoodWait t_trial+sd.RawEvents.Trial{k}.States.PlayTone(1)]; % trigger
        events.Outcomes = [events.Outcomes, 'Hit'];
    else
        events.Outcomes = [events.Outcomes, 'Miss'];    
    end
       
    if ~isempty(sd.Custom.BadPokeTime{k})
        badpokes = [sd.RawEvents.Trial{k}.States.ITI_Poke; sd.RawEvents.Trial{k}.States.Trace_Poke];
...
events.Triggers = events.Triggers  - t0;
% figure;
