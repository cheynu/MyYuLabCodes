function EventOut = AlignMED2Apollo(EventOut, bMED)
% 2023.2.23 Yu Chen
% Adapted to Biosignal Apollo II used for mouse ephys
% There's only rising moment of events in Biosignal system
...
if any(ismember(EventOut.EventsLabels,'Trigger'))
    Tone_ephys = EventOut.Offset{strcmp(EventOut.EventsLabels, 'Trigger')};
else
    Tone_ephys = NaN;
end
...
IndexEphysMED       = findseqmatchrev(leverpress_MED, leverpress_ephys, 0, 0);
...
text(leverpress_ephys(1)-500, 5.2, 'Biosignal press')
plot(leverpress_EphysMED, 4.5, 'ro');
text(leverpress_ephys(1)-500, 4.7, 'Mapped')
line([leverpress_EphysMED; leverpress_ephys'], [4.5 5], 'color', 'b','Linestyle','--');
...
if isnan(leverrelease_ephys)
    leverrelease_ephys = leverpress_ephys + (leverrelease_EphysMED(:) - leverpress_EphysMED(:));
    EventOut.Offset{strcmp(EventOut.EventsLabels, 'LeverPress')} = leverrelease_ephys;
end
% subtract 5
plot(leverrelease_MED, -1+3, 'ko')
text(leverrelease_MED(1)-500, -1.3+3, 'MED release')
hold on
plot(leverrelease_ephys, 0+3, 'r^');
text(leverrelease_ephys(1)-500, 0.2+3, 'Biosignal release')
plot(leverrelease_EphysMED, -0.5+3, 'ro');
text(leverrelease_ephys(1)-500, -0.3+3, 'Mapped')
line([leverrelease_EphysMED; leverrelease_ephys'], [4.5 5]-5+3, 'color', 'b','Linestyle','--')
...
if isnan(Tone_ephys)
    ...
    %     ind_pre = find(leverpress_MED<=tTone, 1, 'last'); % this is the index for last lever press preceeding this tone
        ...
        t_tonepress = [t_tonepress Tone_ephys(k)-leverpress_ephys_k];  % this is the Foreperiod
    end
else % have recorded Trigger info in ephys device
    for k=1:length(Tone_ephys)
        leverpress_ephys_k = leverpress_ephys(find(leverpress_ephys<Tone_ephys(k),1,'last'));
        t_tonepress = [t_tonepress Tone_ephys(k) - leverpress_ephys_k];
    end
end
...
idxTrigger = find(strcmp(EventOut.EventsLabels, 'Trigger'), 1);
if isempty(idxTrigger)
    EventOut.EventsLabels{end+1}='Trigger';
    EventOut.Onset{end+1} = Tone_ephys;
else
    EventOut.Onset{idxTrigger} = Tone_ephys;
    EventOut.Offset{idxTrigger} = EventOut.Onset{idxTrigger} + 250; %ms
end
