...
medttl_bpod = BpodEvents.GoodRelease.*1000;
medttl_ephys = EventOut.Onset{strcmp(EventOut.EventsLabels, 'GoodRelease')};

valve_bpod = BpodEvents.Reward(1,:).*1000;
valve_ephys = EventOut.Onset{strcmp(EventOut.EventsLabels, 'Valve')};
...
if length(press_bpod)>15 & length(press_ephys)>15 
    % use press event to align
    seqmom = press_bpod;
    seqson = press_ephys;
elseif abs((length(medttl_bpod)-length(medttl_ephys))-(length(valve_bpod)-length(valve_ephys)))<15
    % use medttl (good release) to align
    seqmom = medttl_bpod;
    seqson = medttl_ephys;
else
    % use valve-on to align
    seqmom = valve_bpod;
    seqson = valve_ephys;
end
% 
