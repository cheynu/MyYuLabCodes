classdef KilosortOutputClass<handle
    ...
    methods
        ...
        function r = buildRNeuropixelsGPS(obj, varargin)
            %% Set BuildR params
            KornblumStyle = false;
            ProbeStyle = true;
            Subject = 'Michael';
            ...
            %% Check spks
            % check spike units on each channel
            units = {};
            k = 1;
            while k<=height(obj.SpikeTable)
                channel = obj.SpikeTable(k,:).ch{1};
            ...
            if isempty(BehClassPath)
                beh_file = dir("./*BehSessionClass*.mat");
                if isempty(beh_file)
                    error("did not find a behavior class file in current directory.");
                elseif length(beh_file)>1
                    error("found more than one behavior class file in current directory.");
                end
                BClass = load(beh_file.name);
            else
                BClass = load(BehClassPath);
            end
            BehClass = BClass.obj; clear BClass;
            BehClass.print("./");
            ...
            %%
            % a name for saving the r array
            aGoodName = 'RTarray_'+BehClass.Subject+'_'+BehClass.Session+'.mat';

            %% Align behavior to ephys
            EventOut.Meta.Subject = Subject;
            EventOut.Meta.Experimenter = Experimenter;
            ...
            figure(21); clf
            axes('nextplot', 'add', 'ylim', [0 7]);

            for i = 1:length(r.Behavior.Labels)
                ...
                ind_event = ones(length(event_onset), 1) * i;
                r.Behavior.EventTimings = [r.Behavior.EventTimings; event_onset];
                r.Behavior.EventMarkers = [r.Behavior.EventMarkers; ind_event];

                plot(event_onset, i, plot_marker{i});
                text(event_onset(1), i+.2, i_label);
            end
...
%             clc
        ...
        function r = buildRNeuropixels(obj, varargin)
            ...
            if isempty(b) || isempty(BehaviorClass)
                ...
                if any(contains(lower(BpodProtocol),lower({'SRTL_02_MedLick_recording','LeverPressVI'})))
            ...
            % return FP if there is no FP (wait 1/2 sessions)
            if isempty(b.FPs) & ~contains(lower(b.Metadata.ProtocolName),lower({'LeverPressBpod','LeverReleaseBpod'}))
                b = UpdateWaitB(b); % add FP
            ...
            BpodFile = dir([Subject '*_Med*.mat']);
            if isempty(BpodFile)
                BpodFile = dir([Subject '*_Lever*.mat']);
            end
            BpodFilenames = {BpodFile.name};
            for i_file = 1:length(BpodFilenames)
                load(BpodFilenames{i_file});
                ...
                switch BpodProtocol
                    case 'OptoRecording'
                        ...
                        BpodEvents = Bpod_Events_MedOptoRecording(SessionData);
                        % Update poke based on data from Bpod events
                        EventOutCombined = Spikes.Array.UpdatePokeFromBpodEvents(EventOutCombined, BpodEvents);
                    case {'OptoRecordingMix',  'MedOptoRecMixKB'}  % optogenetic stimulation was applied at the onset of different events, we need to extract those times, and align them to blackrock's time
                        %  read bpod events
                        BpodEvents = Bpod_Events_MedOptoRecMix(SessionData);
                        EventOutCombined = UpdateDIOMedOptoRecMix(EventOutCombined, BpodEvents);
                    case 'OptoRecordingSelfTimed'
                        ...
                        % Update poke based on data from Bpod events
                        EventOutCombined = Spikes.Array.UpdatePokeFromBpodEvents(EventOutCombined, BpodEvents);
                    case {'LeverPressVI','LeverpressVI'}
                        vb = Behavior.LeverPressVI.track_training(BpodFilenames{i_file});
                        BpodEvents = Bpod_Events_LeverPressVI(vb);
                        % BpodEventsOld = Bpod_Events_LeverPressVI_BackUp(SessionData,vb);
                        if isempty(EventOutCombined.Onset{contains(EventOutCombined.EventsLabels,'Trigger')})
                            if contains(BehaviorClass.Protocol, 'LeverPressBpod')
                                % BehaviorClass.ToneTime-BehaviorClass.PressTime=0.01s
                                EventOutCombined.Onset{contains(EventOutCombined.EventsLabels,'Trigger')} = EventOutCombined.Onset{contains(EventOutCombined.EventsLabels,'LeverPress')}+10;
                                EventOutCombined.Offset{contains(EventOutCombined.EventsLabels,'Trigger')} = EventOutCombined.Onset{contains(EventOutCombined.EventsLabels,'Trigger')}+250;% 250ms tone
                            end
                        end
                        EventOutCombined = Spikes.Array.UpdatePokeFromBpodEventsLeverpressVI(EventOutCombined, BpodEvents);
                    case 'SRTL_02_MedLick_recording'
                        ...
                        if strcmp(BehaviorClass.Protocol, 'LeverPressBpodEphys')
                            EventOutCombined.Onset{strcmp(EventOutCombined.EventsLabels, 'Trigger')} = EventOutCombined.Onset{strcmp(EventOut.EventsLabels, 'LeverPress')}+10;% BehaviorClass.ToneTime-BehaviorClass.PressTime=0.01s
                            EventOutCombined.Offset{strcmp(EventOutCombined.EventsLabels, 'Trigger')} = EventOutCombined.Onset{strcmp(EventOutCombined.EventsLabels, 'Trigger')}+250; % 250ms tone
                        end
                        EventOutCombined = UpdatePokeFromBpodEventsMedLick(EventOutCombined, BpodEvents);
                end
            end

            EventOutCombined = Spikes.Array.AlignBehaviorClassToBR_CY(EventOutCombined, BehaviorClass, BpodEvents);
        ...
        function r = buildRNeuropixels_AutoShaping(obj, varargin)
            ...
            for i_file = 1:length(BpodFilenames)
                ...
                % Align Events
                switch BpodProtocol
                    case 'SRTL_01_AutoShaping_purebpod'
                    ...
                    case 'SRTL_01_AutoShapingTrace_purebpod'
                        BpodEvents = Bpod_Events_AutoShapingTrace_purebpod(SessionData);
                        EventOutCombined = UpdatePokeFromBpodEventsAutoshaping(EventOutCombined, BpodEvents);
                end
            end
            ...
            r.Behavior.Outcome                        = EventOutCombined.Outcome;
            % % the followings are redundant but are listed so that other
            % % programs still work.
            % r.Behavior.CorrectIndex                 =      find(strcmp(r.Behavior.Outcome, 'Correct'));
            % r.Behavior.PrematureIndex            =      find(strcmp(r.Behavior.Outcome, 'Premature'));
            ...
            pat = digitsPattern(8) + "_" + digitsPattern(6);
            datechar = char(extract(BpodFile.name,pat));
            dt = datetime(datechar, 'InputFormat', 'yyyyMMdd_HHmmss');
        ...
        function plotChannelActivity(obj)
            ...
            EasyPlot.exportFigure(fig, sprintf('channelActivity_s%d_m%d.png', num_s, num_m));
        end
