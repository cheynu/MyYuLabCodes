...
for curUnitInd=1:numUnits
    ...
    for curSpikeTime = 1:min([gwfparams.nWf curUnitnSpikes])
        if spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(end) <= nSamp
            ...
            tmpWf = mmf.Data.x(1:gwfparams.nCh,spikeTimeKeeps(curUnitInd,curSpikeTime)+gwfparams.wfWin(1):end);
            waveForms(curUnitInd,curSpikeTime,:,1:size(tmpWf,2)) = tmpWf(chMap,:);
        end
    end
