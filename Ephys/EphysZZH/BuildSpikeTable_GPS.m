...
while k <= height(spikeTable)
    if isempty(spikeTable(k,:).group{1}) || strcmp(spikeTable(k,:).group{1},'noise') || strcmp(spikeTable(k,:).group{1},'unsorted')
        spikeTable(k,:) = [];
    else
        k=k+1;
    end
...
for k = 1:height(spikeTable)
    ...
    if n_waveform>1
        ...
        raw_waveforms = nan(size(wf.waveForms, 2:4));
        raw_waveforms(:,:,:) = wf.waveForms(1,:,:,:);
        raw_waveforms = permute(raw_waveforms, [2,1,3]);
    end
