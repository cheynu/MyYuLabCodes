...
for k = 1:height(spikeTable)
    ...
    % Number of waveforms per unit to pull out
    gwfparams.nWf           = length(spikeTable(k,:).spike_times{1});
    % Vector of cluster spike times (in samples), same length as .spikeClusters
    gwfparams.spikeTimes    = spikeTable(k,:).spike_times{1};
    % Vector of cluster IDs (Phy nomenclature), same length as .spikeTimes
    gwfparams.spikeClusters = ones(length(spikeTable(k,:).spike_times{1}),1); 
...
for k = 1:length(NS6all)
    NS6 = NS6all(k);
    if k > 1
        % start time of this session relative to the first session
        dt_i = NS6all(k).MetaTags.DateTimeRaw-NS6all(1).MetaTags.DateTimeRaw;
        dBlockOnset=dt_i(end)+dt_i(end-1)*1000+dt_i(end-2)*1000*60+dt_i(end-3)*1000*60*60;
    else
...
KilosortOutput.buildR(...
    ...
    'Blocks', Blocks, ...
    'DIOVersion', "Version4", ...
    'MEDProtocol', MEDProtocol, ...
    'BpodProtocol', "MedOptoRecording");
%% Uncommenented correspoding segment to plot PSTHs

load(dir("RTarrayAll*.mat").name);

for i = 1:length(r.Units.SpikeTimes)
    switch MEDProtocol
        case {"AutoShaping"}
            SRTSpikesLearningAutoShaping(r, i);
        case {"LeverPress", "LeverRelease"} 
            SRTSpikesLearningNaive(r, i);
        case {"Wait1", "Wait2", "Wait"}
            SRTSpikesLearningWait(r, i);
        case {"2FPs", "3FPs"}
            SRTSpikesLearningWait(r, i);
    end
    close all;
