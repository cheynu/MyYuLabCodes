function plotSpikesDCZ4Blocks(r, unit_num, dose, options)

arguments
    r
    unit_num
    dose
    options.drugSegment = 2
    options.halfrecoverySegment = length(r.Meta)-1
    options.recoverySegment = length(r.Meta) % last
    options.t_pre_press = -2500
    options.t_pre_release = -500
    options.t_pre_reward = -1000
    options.t_post_press = 2000
    options.t_post_release = 1000
    options.t_post_reward = 10000
    options.ax_width = 1/1000
    options.ax_height = 3/50
    options.FPselect = 1500 % only analyze the trials of FP 1500 ms
end
drugSegment = options.drugSegment;
halfrecoverySegment = options.halfrecoverySegment;
recoverySegment = options.recoverySegment;
t_pre_press = options.t_pre_press;
t_pre_release = options.t_pre_release;
t_pre_reward = options.t_pre_reward;
t_post_press = options.t_post_press;
t_post_release = options.t_post_release;
t_post_reward = options.t_post_reward;
ax_width = options.ax_width;
ax_height = options.ax_height;
FPselect = options.FPselect;

if ~isempty(recoverySegment)
    ifRecovery = true;
    if recoverySegment~=halfrecoverySegment && drugSegment~=halfrecoverySegment
        ifHalfRecovery = true;
    else
        ifHalfRecovery = false;
    end
else
    ifRecovery = false;
    ifHalfRecovery = false;
...
if ifHalfRecovery
    ax_raster_halfrecovery = EasyPlot.copyAxes(ax_raster_post, 'bottom');
    EasyPlot.set(ax_raster_halfrecovery, 'XAxisVisible', 'off', 'YDir', 'reverse');
    EasyPlot.set(ax_raster_halfrecovery(2:3), 'YAxisVisible', 'off', 'YDir', 'reverse');

    ax_raster_recovery = EasyPlot.copyAxes(ax_raster_halfrecovery, 'bottom');
    ...
    ax_raster_all = {ax_raster_pre, ax_raster_post, ax_raster_halfrecovery, ax_raster_recovery};
elseif ifRecovery
...
elseif ~ifHalfRecovery
    ...
    t_end_segment_all = [t_end_control_session, t_end_drug_session, get_t_end_session(r, recoverySegment)];
else % ifHalfRecovery
    t_end_drug_session = get_t_end_session(r, halfrecoverySegment-1);
    t_start_segment_all = [0, t_start_drug_session, get_t_start_session(r, halfrecoverySegment), get_t_start_session(r, recoverySegment)];
    t_end_segment_all = [t_end_control_session, t_end_drug_session, get_t_end_session(r, halfrecoverySegment), get_t_end_session(r, recoverySegment)];
end
blk_duration = t_end_segment_all - t_start_segment_all; 
...
if ifHalfRecovery
    postTime = [get_t_start_session(r, halfrecoverySegment) get_t_start_session(r, recoverySegment)];
%     ax_firing_rate_all{1} = ax_firing_rate;
%     for k=1:length(r.Meta)-1
%         [ax_firing_rate_all{k}, ax_firing_rate_all{k+1}] = EasyPlot.truncAxis(ax_firing_rate_all{k},...
%             "X", [get_t_end_session(r,k)/1000/60, get_t_start_session(r,k+1)/1000/60],...
%             'Xratio', blk_duration(k)./sum(blk_duration(k+1:end)),...
%             'truncRatio', 0.2/ax_firing_rate_all{k}.Position(3));
%     end
    
    [ax_firing_rate1, ax_firing_rate2_All] = EasyPlot.truncAxis(ax_firing_rate,...
        'X', ([t_end_drug_session, postTime(1)]-drugTime)./1000./60,...
        'Xratio',t_end_drug_session./sum(blk_duration(halfrecoverySegment:end)),...
        'truncRatio',0.06/ax_firing_rate.Position(3));
    [ax_firing_rate2, ax_firing_rate3] = EasyPlot.truncAxis(ax_firing_rate2_All,...
        'X', ([t_end_segment_all(end-1), postTime(2)]-drugTime)./1000./60,...
        'Xratio',diff([postTime(1),t_end_segment_all(end-1)])/sum(blk_duration(recoverySegment:end)),...
        'truncRatio',0.06/ax_firing_rate2_All.Position(3));
%     [ax_firing_rate1, ax_firing_rate2_All] = EasyPlot.truncAxis(ax_firing_rate,...
%         'X', ([t_end_drug_session, postTime(1)]-drugTime)./1000./60,...
%         'Xratio',t_end_drug_session./diff([postTime(1),t_end_segment_all(end)]),...
%         'truncRatio',0.2/ax_firing_rate.Position(3));
%     [ax_firing_rate2, ax_firing_rate3] = EasyPlot.truncAxis(ax_firing_rate2_All,...
%         'X', ([t_end_segment_all(end-1), postTime(2)]-drugTime)./1000./60,...
%         'Xratio',diff([postTime(1),t_end_segment_all(end-1)])/diff([postTime(2),t_end_segment_all(end)]),...
%         'truncRatio',0.2/ax_firing_rate2_All.Position(3));
    xlim(ax_firing_rate1, ([t_start_segment_all(1), t_end_drug_session]-drugTime)/1000/60);
    xlim(ax_firing_rate2, ([postTime(1), t_end_segment_all(end-1)]-drugTime)/1000/60);
    xlim(ax_firing_rate3, ([postTime(2), t_end_segment_all(end)]-drugTime)/1000/60);
    ax_firing_rate_all = {ax_firing_rate1, ax_firing_rate2, ax_firing_rate3};
elseif ifRecovery
...
if ifHalfRecovery
    EasyPlot.setGeneralTitle(ax_raster_halfrecovery, [num2str((postTime(1)-drugTime)/1000/60/60, '%.1f'), ' hour after injection'], 'yShift', -0.2);
    EasyPlot.setGeneralTitle(ax_raster_recovery, [num2str((postTime(2)-drugTime)/1000/60/60, '%.1f'), ' hour after injection'], 'yShift', -0.2);
elseif ifRecovery
...
[~,~] = mkdir('./Fig');
pathSave = fullfile('./Fig', [rat_name, '_', session, '_', 'Unit', num2str(unit_num)]);
EasyPlot.exportFigure(fig, pathSave);
end
