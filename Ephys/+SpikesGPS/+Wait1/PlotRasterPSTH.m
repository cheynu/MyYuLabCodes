...
ind_fp       = strcmp(PSTH.CentInLabels, 'FP');

% PSTH of correct trials
y_col1_row1 = 1;
ha_cent_in_psth = cell(NumStages, 1);
for i = 1:NumStages
    yshift = y_col1_row1 + (h_psth+0.1)*(NumStages-i);
    ...
    if Stages(i)
        xline(TargetFP*1000, 'Color', c_trigger, 'LineWidth', 1, 'Alpha', 1);
    end
    for j = 1:NumPorts
...
% make raster plot
y_col1_row2 = y_col1_row1 + (h_psth+.1)*NumStages;
% Plot spike raster of correct trials (all FPs)
ntrials_cent_in_ij = zeros(NumStages, NumPorts);
for i = 1:NumStages
    for j = 1:NumPorts
...
for m = 1:NumStages
    for n = 1:NumPorts
        ...
        fp      = PSTH.CentIn{m,n}{ind_fp};

        xx_all = []; % spike times
        yy_all = [];
        xxrt_all = []; % reaction (cent-out) times
        yyrt_all = [];
        xxfp_all = []; % reaction (cent-out) times
        yyfp_all = [];
...
ha_cent_in_psth_premature =  axes('unit', 'centimeters', 'position', [x_col1 y_col1_row3 w_psth h_psth], 'nextplot', 'add',...
...
axes('unit', 'centimeters', 'position', [x_col1 y_col1_row4 w_psth ntrial_premature*rasterheight],...
    'nextplot', 'add',...
...
y_col1_row5 = y_col1_row4 + 0.5 + ntrial_premature*rasterheight;
% Add information
...
ind_fp       = strcmp(PSTH.CentOutLabels, 'FP');

% PSTH of correct trials
y_col2_row1 = y_col1_row1;
ha_cent_out_psth = cell(1,NumStages);
for i = 1:NumStages
    yshift = y_col2_row1 + (h_psth+0.1)*(NumStages-i);
...
% Plot spike raster of correct trials (all FPs)
ntrials_cent_out_ij = zeros(NumStages, NumPorts);
for i = 1:NumStages
    for j = 1:NumPorts
...
ha_cent_out_psth_premature =  axes('unit', 'centimeters', 'position', [x_col2 y_col2_row3 w_psth h_psth], 'nextplot', 'add',...
...
axes('unit', 'centimeters', 'position', [x_col2 y_col2_row4 w_psth ntrial_premature*rasterheight],...
    'nextplot', 'add',...
...
uicontrol('Style', 'text', 'Units', 'centimeters', 'Position', [x_col2-.25 y_col2_row5 4 1],...
...
ha_trigger_psth = cell(1,NumStages);
for i = 1:NumStages
    yshift = y_col3_row1 + (h_psth+0.1)*(NumStages-i);
...
% Plot spike raster of correct trials (all FPs)
ntrials_trigger_ij = zeros(NumStages, NumPorts);
for i = 1:NumStages
    for j = 1:NumPorts
...
for m = 1:NumStages
    for n = 1:NumPorts
        ...
        line(xx_all, yy_all, 'color', c_port{n}, 'linewidth', .2);
        line(xxrt_all, yyrt_all, 'color', c_cent_out, 'linewidth', 1);
        line(xxfp_all, yyfp_all, 'color', c_cent_in, 'linewidth', 1);
        scatter(x_choice, y_choice, 8, 'o', 'filled','MarkerFaceColor', c_reward,  'markerfacealpha', 0.5, 'MarkerEdgeColor','none');
    end
end
xline(0, 'Color', c_trigger, 'LineWidth', 1, 'Alpha', 1);
...
% Add information
uicontrol('Style', 'text', 'Units', 'centimeters', 'Position', [x_col3-.25 y_col3_row3 3 1],...
...
ha_choice_psth = cell(1,NumStages);
for i = 1:NumStages
    yshift = y_col4_row1 + (h_psth+0.1)*(NumStages-i);
...
% Plot spike raster of correct trials (all FPs)
ntrials_reward_ij = zeros(NumStages, NumPorts);
for i = 1:NumStages
    for j = 1:NumPorts
...
y_spk_rate = max([y_col3_row3 y_col4_row5]) + 2.5;
