...
ha_cent_out_psth_premature =  axes('unit', 'centimeters', 'position', [x_col2 y_col2_row5 w_psth h_psth], 'nextplot', 'add',...
    'xlim',  [-CentOutTimeDomain(1) CentOutTimeDomain(2)], 'xticklabel', [], 'FontSize', 7, 'TickDir', 'Out');
xline(0, 'Color', c_cent_out, 'LineWidth', 1, 'Alpha', 1);
for j = 1:NumPorts
...
axes('unit', 'centimeters', 'position', [x_col2 y_col2_row6 w_psth ntrial_premature*rasterheight],...
    'nextplot', 'add',...
...
uicontrol('Style', 'text', 'Units', 'centimeters', 'Position', [x_col2-.25 y_col2_row7 4 1],...
...
for n = 1:NumPorts
    ...
    rt      = PSTH.TriggerLate{n}{ind_rt};
    fp      = PSTH.TriggerLate{n}{ind_fp};

    xx_all = []; % spike times
    yy_all = [];
    xxrt_all = []; % cent-out times
    yyrt_all = [];
    xxfp_all = []; % cent-out times
    yyfp_all = [];
    x_choice = []; % choice times
    y_choice = [];
