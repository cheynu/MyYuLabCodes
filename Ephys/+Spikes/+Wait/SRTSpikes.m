...
% 
if ~isequal(length(t_presses),length(t_releases))
    min_n_press = min(length(t_presses),length(t_releases));
    t_presses = t_presses(1:min_n_press);
    t_releases = t_releases(1:min_n_press);
    disp(['Number of presses is ' num2str(length(t_presses))])
end
...
ind_late        =   ind_late(ind_late<=length(t_presses));
