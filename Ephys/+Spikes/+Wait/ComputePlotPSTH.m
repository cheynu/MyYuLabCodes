...
annotation('textbox', ...
    ...
    'Position', [0.5 yshift_row6  6 1], ... % Normalize to figure units
    'String', 'A. Press-related activity', ...
...
if size(r.Units.SpikeNotes, 2) == 4
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ... % Set units to centimeters
        'Position', [1 yshift_row7+1.2 6 1.2], ... % Normalize to figure units
        'String', (['Unit #' num2str(ku) ' (Ch ' num2str(ch) ' | UnitOnCh ' num2str(unit_no) ' | ' 'Kilosort cluster ' num2str(cluster_id) ')']), ...
        'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
        'BackgroundColor', 'w', ... % White background
        'FontSize', 10, ...
        ...
        'FontName', 'DejaVu Sans', ... % Note: MATLAB uses 'DejaVu Sans' (case-sensitive)
        'EdgeColor', 'none', ...
        'HorizontalAlignment', 'center'); % No border, similar to uicontrol default
else
    cluster_id = [];
    % uicontrol('style', 'text', 'units', 'centimeters', 'position', [1 yshift_row7+1.2 6 1.2],...
...
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [1 yshift_row7+2.4 10 0.5], ... % Normalize to figure units
    'String', ([anm_brain ' '  r.BehaviorClass.Date ' | ' r.BehaviorClass.Protocol]), ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
    'FontWeight', 'bold', ...
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ... % Set units to centimeters
        'Position', [1 yshift_row7+2.4 10 0.5], ... % Normalize to figure units
        'String', ([anm_brain ' '  strrep( r.Meta(1).DateTime(1:11), '-', '_') ' | ' r.BehaviorClass.Protocol]), ...
        'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
        'BackgroundColor', 'w', ... % White background
        'FontSize', 10, ...
        'FontWeight', 'bold', ...
...
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [7.75 yshift_row6 width+1 1], ... % Normalize to figure units
    'String', 'B. Release-related', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
...
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [col3-0.5 yshift_row6new 3.5 0.5], ... % Normalize to figure units
    'String', 'D.  Activity vs time', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
...
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [col5 yshift_row7 5 1.5], ... % Normalize to figure units
    'String', 'F. Spike waveform and autocorrelation', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
