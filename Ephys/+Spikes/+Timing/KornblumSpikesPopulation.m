...
% deal with some bugs
n_min = min(length(t_presses),length(t_releases));
t_presses = t_presses(1:n_min);
t_releases = t_releases(1:n_min);
ind_cue(ind_cue>n_min) = [];
ind_uncue(ind_uncue>n_min) = [];
rb.CorrectIndex(rb.CorrectIndex>n_min) = [];
...
ylabel('Number of presses')
 ...
 % release for cue and uncue trials
