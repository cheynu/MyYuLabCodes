...
% pre-filter & align press & release (because of the bug that they may be different length
maxT = max(max(r.Units.SpikeTimes(ku).timings),max(t_presses)+5000);
ind_press = t_presses-params_press.pre>0 & t_presses+params_press.post < maxT;
t_presses = t_presses(ind_press);
maxT = max(max(r.Units.SpikeTimes(ku).timings),max(t_releases)+5000);
ind_release = t_releases-params_releases.pre>0 & t_releases+params_releases.post < maxT;
t_releases = t_releases(ind_release);
[t_presses, t_releases, idxIn, idxOut] = matchSeqOnsetOffset(t_presses, t_releases);
idxValid = ~isnan(idxOut);
t_presses = t_presses(idxValid);
t_releases = t_releases(idxValid);
...
if size(r.Units.SpikeNotes, 2) == 4
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ...
        'Position', [1 yshift_row4+2 6 1.2], ...
        'String', (['Unit #' num2str(ku) ' (Ch ' num2str(ch) ' | UnitOnCh ' num2str(unit_no) ' | ' 'Kilosort cluster ' num2str(cluster_id) ')']), ...
        'FontName', 'Dejavu Sans', ...
        'FontWeight', 'bold', ...
        'FontSize', 10, ...
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [1 yshift_row4+3.2 6 0.5], ...
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [7.75 yshift_row4 width+1 1], ...
    'String', 'B. Release-related', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
    'FontSize', 10, ...
...
%% add location
ha_loc =  axes('unit', 'centimeters', 'position', [8.5 yshift_row4 2 3], 'nextplot', 'add');
