...
trigger_label = {'Triggers', 'TriggersHit', 'TriggersMiss'};
for it = flip(1:length(PSTHOut.Triggers.Time))
    t_triggers = PSTHOut.Triggers.Time{it};
    ...
    PSTH.(trigger_label{it}) =  {psth_trigger, ts_trigger, trialspxmat_trigger,...
        tspkmat_trigger, t_triggers};
end
% PSTH.TriggerLabels = {'PSTH', 'tPSTH', 'SpikeMat', 'tSpikeMat', 'tEvents'};
PSTH.TriggerLabels = {'psth_trigger', 'ts_trigger', 'trialspxmat_trigger', 'tspkmat_trigger', 't_triggers'};
...
plot(ts_nonreward_pokes, psth_nonreward_pokes, 'color', miss_col, 'linewidth', .5);
...
for i =1:ntrials_rewardpoke
    ...
    i_trigger = PSTHOut.Triggers.Time{1}-itreward;
    i_LED = i_trigger+2250;
    i_trigger = i_trigger(i_trigger>=-RewardTimeDomain(1) & i_trigger<=RewardTimeDomain(2));
    i_LED = i_LED(i_LED>=-RewardTimeDomain(1) & i_LED<=RewardTimeDomain(2));
    if ~isempty(i_portin)
    ...
    if ~isempty(i_trigger)
        i_trigger = reshape(i_trigger,1,[]);
        x_trigger = [x_trigger, i_trigger,i_trigger, NaN];
        y_trigger = [y_trigger, yy2, NaN];
    end
    if ~isempty(i_LED)
        i_LED = reshape(i_LED,1,[]);
        x_LED = [x_LED, i_LED,i_LED, NaN];
        y_LED = [y_LED, yy2, NaN];
    end
    k = k+1;
end
...
line(x_trigger, y_trigger, 'color', trigger_col, 'linewidth', 1.5)
line(x_LED, y_LED, 'color', press_col, 'linewidth', 1.5)
scatter(x_portin, y_portin, 8, 'o', 'filled','MarkerFaceColor', reward_col,  'markerfacealpha', 0.9, 'MarkerEdgeColor','none')
% scatter(x_trigger, y_trigger, 8, 'o', 'filled','MarkerFaceColor', trigger_col,  'markerfacealpha', 0.9, 'MarkerEdgeColor','none')
% scatter(x_LED, y_LED, 8, 'o', 'filled','MarkerFaceColor', press_col,  'markerfacealpha', 0.9, 'MarkerEdgeColor','none')
...
line(xx_all, yy_all, 'color', miss_col, 'linewidth', 1)
line(x_trigger, y_trigger, 'color', trigger_col, 'linewidth', 1.5)
line(x_LED, y_LED, 'color', press_col, 'linewidth', 1.5)
% line(x_move_all, y_move_all, 'color', release_col, 'linewidth', 1)
scatter(x_portin, y_portin, 8, 'o', 'filled','MarkerFaceColor', reward_col,  'markerfacealpha', 0.9, 'MarkerEdgeColor','none')
...
%     'string', 'B. Rewarded/Nonrewarded poke-related activity', ...
...
annotation('textbox', ...
    ...
    'Position', [col3-0.5 yshift_row4new 5 1.75], ... % Normalize to figure units
    'String', 'B. Rewarded/Nonrewarded poke-related activity', ...
    ...
    'EdgeColor', 'none', ...
    'HorizontalAlignment', 'left'); % No border, similar to uicontrol default
...
% Add information  13.5 3+0.5 6 ntrial4*rasterheight
% uicontrol('Style','text','Units','centimeters','Position',[col3-0.5 yshift_row6new 6 0.5],...
%     'string', 'C.  Trigger activity vs time', ...
%     'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],'ForegroundColor', 'k', ...
%     'HorizontalAlignment','Left');
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [col3-0.5 yshift_row6new 6 0.5], ... % Normalize to figure units
    'String', 'C.  Trigger activity vs time', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
...
% for i=1:nFPs
plot(PSTH.TriggersHit{2}, PSTH.TriggersHit{1}, 'color', FP_cols(3, :), 'linewidth', 1.5);
plot(PSTH.TriggersMiss{2}, PSTH.TriggersMiss{1}, 'color', miss_col, 'linewidth', 1);
FRMax = max([FRMax max(PSTH.TriggersHit{1}) max(PSTH.TriggersMiss{1})]);
%     disp(FRMax)
% end
% plot(ts_late_trigger, psth_late_trigger, 'color', late_col, 'linewidth', .5)
xlabel('Time from tone stimulus (ms)')
ylabel ('Spks per s')
...
ntrials_trigger = size(PSTH.TriggersHit{3}, 2);
...
ap_mat = PSTH.TriggersHit{3};
t_mat = PSTH.TriggersHit{4};
...
for i =1:ntrials_trigger
    ...
    % port access time
    it_trigger =PSTH.TriggersHit{5}(i);
...
scatter(x_portin, y_portin, 8, 'o', 'filled','MarkerFaceColor', reward_col,  'markerfacealpha', 0.9, 'MarkerEdgeColor','none');
plotshaded([0 250], [get(gca, 'ylim');get(gca, 'ylim')]', trigger_col);
% line([0 0], get(gca, 'ylim'), 'color', trigger_col, 'linewidth', 1);
line([0 0]+2250, get(gca, 'ylim'), 'color', press_col, 'linewidth', 1);
title('Hit', 'fontsize', 7);
axis off

% trigger following miss
ntrials_trigger_miss = size(PSTH.TriggersMiss{3}, 2);
axes('unit', 'centimeters', 'position', [col4 yshift_row3 width ntrials_trigger_miss*rasterheight],...
    'nextplot', 'add', 'xlim', [-TriggerTimeDomain(1) TriggerTimeDomain(2)], 'ylim', [-ntrials_trigger_miss 1], ...
    'box', 'on', 'xticklabel', []);
yshift_row4 = yshift_row3+ntrials_trigger_miss*rasterheight+0.5;
k =0;
ap_mat = PSTH.TriggersMiss{3};
t_mat = PSTH.TriggersMiss{4};
xx_all = [];
yy_all = [];
xxrt_all = [];
yyrt_all = [];
...
for i =1:ntrials_trigger_miss
    ...
    % port access time
    it_trigger =PSTH.TriggersMiss{5}(i);
    i_portin = t_portin - it_trigger;
    i_portin = i_portin(i_portin>=-TriggerTimeDomain(1) & i_portin<=TriggerTimeDomain(2));
    if ~isempty(i_portin)
        i_portin = reshape(i_portin,1,[]);
...
% line([0 0], get(gca, 'ylim'), 'color',trigger_col, 'linewidth', 1)
title('Miss', 'fontname', 'dejavu sans', 'fontsize', 7)
axis off

% Add information
% uicontrol('Style','text','Units','centimeters','Position',[col4-0.5  yshift_row4 5 1.2],...
%     'string', 'A. Trigger-related activity', ...
%     'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],'ForegroundColor', 'k', ...
%     'HorizontalAlignment','Left');
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [col4-0.5  yshift_row4 5 1.2], ... % Normalize to figure units
    'String', 'A. Trigger-related activity', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
...
axes(ha_trigger)
plotshaded([0 250], [FRrange;FRrange]', trigger_col);
% line(ha_trigger, [0 0], FRrange, 'color', trigger_col, 'linewidth', 1);
line(ha_trigger, [2250 2250], FRrange, 'color', press_col, 'linewidth', 1);
...
%     'string', 'D. Spike waveform and autocorrelation', ...
%     'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],'ForegroundColor', 'k', ...
%     'HorizontalAlignment','Left');
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [col5-0.5 yshift_row7 5 1.5], ... % Normalize to figure units
    'String', 'D. Spike waveform and autocorrelation', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
...
if size(r.Units.SpikeNotes, 2) == 4
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ... % Set units to centimeters
        'Position', [1 yshift_row7 6 1.2], ... % Normalize to figure units
        'String', (['Unit #' num2str(ku) ' (Ch ' num2str(ch) ' | UnitOnCh ' num2str(unit_no) ' | ' 'Kilosort cluster ' num2str(cluster_id) ')']), ...
        'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
        'BackgroundColor', 'w', ... % White background
        'FontSize', 10, ...
...
if isfield(r, 'BehaviorClass')
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ... % Set units to centimeters
        'Position', [1 yshift_row7+1.2 6 0.5], ... % Normalize to figure units
        'String', ([anm_brain ' '  num2str(r.BehaviorClass.Date)]), ...
        'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
        'BackgroundColor', 'w', ... % White background
        'FontSize', 10, ...
        'FontWeight', 'bold', ...
...
loxX = unique(ChanMap.xcoords);
loc_x = ChanMap.xcoords(ch);
loc_x_ind = find(loxX == loc_x);
loc_y = ChanMap.ycoords(ch);
y_min = min(ChanMap.ycoords);
y_max = max(ChanMap.ycoords);
x_positions = [1, 2, 3, 4];
