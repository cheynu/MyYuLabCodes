...
t_triggers_hit = t_triggers(strcmp(rb.Outcome, 'Hit'));
t_triggers_miss = t_triggers(strcmp(rb.Outcome, 'Miss'));
...
dt                                     =          []; % poke leading to reward
    ...
    for i=1:length(t_rewards)
        ...
        t_portin_this = t_portin(t_portin >t_rewards(i)-50 & t_portin<t_rewards(i)+50);
...
PSTHOut.Triggers.Labels                                  =       {'AllTriggerTime' 'TriggerTime_Hit' 'TriggerTime_Miss'};
PSTHOut.Triggers.Time                                     =       {t_triggers,t_triggers_hit,t_triggers_miss};
...
for iku =1:length(ku_all)
    ...
    PSTHOut.PSTH(iku)       = Spikes.AutoShapingTrace.ComputePlotPSTH(r, PSTHOut, ku,...
