...
TriggerMissPSTHs = cell(1, length(Pop.Reward));
TriggerMissPSTHZs = cell(1, length(Pop.Reward));
tTriggerMissPSTHs = cell(1, length(Pop.Reward));
...
for ifp = 1:nFP
    ...
    thisPSTH                            =       Pop.TriggerMissZ{ifp}(idx_units, :);
    TriggerMissPSTHZs{ifp}               =       thisPSTH(IndSort, :);
    thisPSTH                            =       Pop.TriggerMiss{ifp}(idx_units, :);
    TriggerMissPSTHs{ifp}                 =       thisPSTH(IndSort, :);
    tTriggerMissPSTHs{ifp}              =       Pop.TriggerMissZ{ifp}(1, :);
...
% only miss
IndTriggerMiss = cell(1, nFP);
PSTH_ConcatenateMiss = cell(1, nFP);
PSTH_ConNormMiss = cell(1, nFP);
...
for ifp = 1:length(Pop.Reward)
    ...
    PSTH_ConcatenateMiss{ifp} = TriggerMissPSTHs{ifp};
    PSTH_ConNormMiss{ifp} = normalize(PSTH_ConcatenateMiss{ifp}' , 'range', norm_range);
    PSTH_ConNormMiss{ifp}          =       PSTH_ConNormMiss{ifp}';
    IndTriggerMiss{ifp}                    =    [1:size(TriggerMissPSTHs{ifp}, 2)];
end
...
hf=48;
figure(hf); clf(hf)
set(gcf, 'unit', 'centimeters', 'position', [2 2 22 20], 'paperpositionmode', 'auto' ,'color', 'w')
size_factor = 1.5;
space = 0.1;
xlevel_start = 1.25;
% A. Trigger
ylevel_start = 2;
map_height = 3;
tRange = [max(tTriggerPSTHs{ifp}(1), -3000) 4000];
...
for ifp =1:nFP
    ha_Trigger(ifp) = axes('unit', 'centimeters', 'position',...
        ...
        'xlim', tRange, 'xtick', [-3500:500:4000],...
    ...
    line([0 0]+2250, yrange, 'color', press_col, 'linestyle', ':', 'linewidth', 1.5);
    ...
    title(sprintf('HitTrials  N =%2.0d', Pop.Trials{1} ));
    ha_Trigger(ifp).TitleHorizontalAlignment = 'left';
end
ylevel_now = ylevel_start+(map_height+0.5)*(nFP-1)+map_height+0.5;

for ifp =1:nFP
    ha_TriggerMiss(ifp) = axes('unit', 'centimeters', 'position',...
        [xlevel_start ylevel_now  Width map_height], 'nextplot', 'add',...
        'xlim', tRange, 'xtick', [-3500:500:4000],...
        'ylim', [0.5 n_unit+0.5], 'ydir', 'reverse', 'ticklength', [0.025 0.01], 'XTickLabelRotation', 90);
   
        set(ha_TriggerMiss(ifp), 'xticklabel', [])

    TriggerMissPSTH_thisFP = PSTH_ConNormMiss{ifp}(:, IndTriggerMiss{ifp});
    himage1 = imagesc(tTriggerMissPSTHs{ifp}, [1:n_unit], TriggerMissPSTH_thisFP, norm_range);
    colormap('Parula')
    yrange = [0.5 n_unit+0.5];
    line([0 0], yrange, 'color', trigger_col, 'linestyle', ':', 'linewidth', 1.5);
    line([0 0]+2250, yrange, 'color', press_col, 'linestyle', ':', 'linewidth', 1.5);
    ...
    % line([0 0]+MixedFPs(ifp), yrange, 'color', 'm', 'linestyle', ':', 'linewidth', 1.5)
    plotshaded([get(gca, 'xlim')], [nsig+.5 nsig+.5; n_unit+0.5 n_unit+0.5],  'w', 0.75)
    title(sprintf('MissTrials  N =%2.0d', Pop.Trials{2} ));
    ha_TriggerMiss(ifp).TitleHorizontalAlignment = 'left';
end
ylevel_now = ylevel_start+(map_height+0.5)*(nFP)+map_height+0.5;
% uicontrol('Style','text','Units','centimeters','Position',[xlevel_start ylevel_now  6 0.7],...
...
annotation('textbox', ...
    ...
    'Position', [xlevel_start ylevel_now  6 0.7], ... % Normalize to figure units
    'String', ['A. Normalized activity ([0-1])'], ...
...
% Trigger
% A. Trigger
xlevel_start = xboundbar+3;
ylevel_start = 2;
map_height = 3;
tRange = [max(tTriggerPSTHs{1}(1), -3000) 4000];
Width  = size_factor*diff(tRange)/2000;
for ifp =1:nFP
    ha_Trigger(ifp) = axes('unit', 'centimeters', 'position',...
        [xlevel_start ylevel_start+(map_height+0.5)*(ifp-1)  Width map_height], 'nextplot', 'add',...
...
for ifp =1:nFP
    ha_TriggerMiss(ifp) = axes('unit', 'centimeters', 'position',...
        ...
        'xlim', tRange, 'xtick', [-3500:500:4000],...
        'ylim', [0.5 n_unit+0.5], 'ydir', 'reverse', 'ticklength', [0.025 0.01], 'XTickLabelRotation', 90);
    colormap(ha_TriggerMiss(ifp), mycolormap);
    % if ifp >1
        set(ha_TriggerMiss(ifp), 'xticklabel', [])
    % else
    %     ylabel('Units')
    %     xlabel('Trigger (ms)')
    % end;
    imagesc(tTriggerMissPSTHs{ifp} , [1:n_unit],  TriggerMissPSTHZs{ifp}, zrange);
    yrange = [0.5 n_unit+0.5];
    line([0 0], yrange, 'color', trigger_col, 'linestyle', ':', 'linewidth', 1.5);
    line([0 0]+2250, yrange, 'color', press_col, 'linestyle', ':', 'linewidth', 1.5);
    % line([0 0]+MixedFPs(ifp), yrange, 'color', 'm', 'linestyle', ':', 'linewidth', 1.5)
...
ylevel_now = ylevel_start+(map_height+0.5)*(nFP)+map_height+2;
...
%     'Position', [2 ylevel_now 8 3], 'ColumnWidth',{50, 50, 50, 50, 100});

% Add information

% uicontrol('Style','text','Units','centimeters','Position',[11 ylevel_now  6 2],...
%     'string', [Pop.Name ' | ', Pop.UnitsColumn5{ib} ' | ' strrep(Pop.Session, '_', '-')], ...
%     'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 12,'BackgroundColor','w')
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [11 ylevel_now  6 2], ... % Normalize to figure units
    'String', [Pop.Name ' | ', Pop.UnitsColumn5{ib} ' | ' strrep(Pop.Session, '_', '-')], ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 12, ...
