...
switch SortBy
    case 'Time'
        ifSortByHT = false;
    case {'HT','HoldTime','PressDur','PressDuration'}
        ifSortByHT = true;
        SortBy = 'HT';
    otherwise
...
t_presses               = PSTHOut.Presses.TimeCell{end};
[psth_presses_all, ts_press_all, trialspxmat_press_all, tspkmat_press_all,  t_correct_presses_all, ind] ...
                        = Spikes.jpsth(r.Units.SpikeTimes(ku).timings, t_presses, params_press);
...
% Press PSTH (corrected)
params_press.pre        = PressTimeDomain(1);
params_press.post       = PressTimeDomain(2);
params_press.binwidth   = 20;

t_presses_correct       = PSTHOut.Presses.TimeCell{1}; 
[psth_presses_correct, ts_press, trialspxmat_press, tspkmat_press, t_presses_correct, ind] ...
                        = Spikes.jpsth(r.Units.SpikeTimes(ku).timings, t_presses_correct, params_press);
psth_presses_correct    = smoothdata(psth_presses_correct, 'gaussian', 5);

PSTH.Presses            = {psth_presses_correct, ts_press, trialspxmat_press,...
    tspkmat_press, t_presses_correct};
...
t_releases                  = PSTHOut.Releases.TimeCell{end};
[psth_releases_all, ts_releases_all, trialspxmat_releases_all, tspkmat_releases_all, t_correct_releases_all, ind] ...
                            = Spikes.jpsth(r.Units.SpikeTimes(ku).timings, t_releases, params_releases);
...
% Release PSTH (correct)
t_releases_correct          = PSTHOut.Releases.TimeCell{1};
[psth_release_correct, ts_release, trialspxmat_release, tspkmat_release, t_releases_correct, ind] ...
                            = Spikes.jpsth(r.Units.SpikeTimes(ku).timings, t_releases_correct, params_releases);
psth_release_correct        = smoothdata (psth_release_correct, 'gaussian', 5);

PSTH.Releases               = {psth_release_correct, ts_release, trialspxmat_release,...
    tspkmat_release,  t_releases_correct};
PSTH.ReleasesLabels         = {'PSTH', 'tPSTH', 'SpikeMat', 'tSpikeMat', 'tEvents'};

% Premature press PSTH
t_premature_presses             = PSTHOut.Presses.TimeCell{1+1};
t_premature_releases            = PSTHOut.Releases.TimeCell{1+1};
...
[psth_premature_release, ts_premature_release, trialspxmat_premature_release, tspkmat_premature_release,...
    ...
    t_premature_releases, params_releases);
psth_premature_press            = smoothdata(psth_premature_press, 'gaussian', 5);
premature_duration_presses      = t_premature_releases-t_premature_presses;
% premature_duration_presses      = premature_duration_presses(ind);

if ifSortByHT
    [~,IndPremature] = sort(premature_duration_presses);
    trialspxmat_premature_press = trialspxmat_premature_press(:,IndPremature);
    t_premature_presses = t_premature_presses(IndPremature);
    premature_duration_presses = premature_duration_presses(IndPremature);
end

PSTH.PrematurePresses           = {psth_premature_press, ts_premature_press,...
    trialspxmat_premature_press, tspkmat_premature_press,...
    t_premature_presses, premature_duration_presses};
...
premature_duration_releases     = premature_duration_presses;
if ifSortByHT
    trialspxmat_premature_release = trialspxmat_premature_release(:,IndPremature);
    t_premature_releases = t_premature_releases(IndPremature);
end
...
hplot_press = plot(ts_press, psth_presses_correct, 'color', FP_cols(3, :),  'linewidth', 1.5);
...
% make raster plot  
if num2str(length(t_presses))>300
    rasterheight = 0.006;
elseif num2str(length(t_presses))>200
    rasterheight = 0.008;
...
ntrials_press = size(trialspxmat_press, 2);
...
% Paint the foreperiod (revised from for loop)
k=0;
...
x_portin = [];
y_portin = [];
ht_correct = t_releases_correct-t_presses_correct;
ht_all = t_releases - t_presses;
...
title('Rewarded (byTime)', 'fontsize', 7, 'fontweight','bold');
axis off
...
line([0 0], get(gca, 'ylim'), 'color', press_col, 'linewidth', 1)
title(sprintf('Unrewarded (By%s)',SortBy), 'fontsize', 7, 'fontweight','bold')
axis off
...
if size(r.Units.SpikeNotes, 2) == 4
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ...
        'Position', [1 yshift_row6+2 6 1.2], ...
        'String', (['Unit #' num2str(ku) ' (Ch ' num2str(ch) ' | UnitOnCh ' num2str(unit_no) ' | ' 'Kilosort cluster ' num2str(cluster_id) ')']), ...
        'FontName', 'Dejavu Sans', ...
        'FontWeight', 'bold', ...
        'FontSize', 10, ...
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [1 yshift_row6+4 6 0.5], ...
    'String', ([anm_brain ' '  r.BehaviorClass.Date]), ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
    'FontSize', 10, ...
    'BackgroundColor', [1 1 1], ...
    ...
    annotation('textbox', ...
        'Units', 'centimeters', ...
        'Position', [1 yshift_row6+4 6 0.5], ...
        'String', ([anm_brain ' '  strrep( r.Meta(1).DateTime(1:11), '-', '_')]), ...
        'FontName', 'Dejavu Sans', ...
        'FontWeight', 'bold', ...
        'FontSize', 10, ...
        'BackgroundColor', [1 1 1], ...
...
title('Rewarded (byTime)', 'fontsize', 7);
axis off
...
line([0 0], get(gca, 'ylim'), 'color', release_col, 'linewidth', 1)
title(sprintf('Unrewarded (By%s)', SortBy), 'fontsize', 7);
axis off
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [7.75 yshift_row5 width+1 1], ...
    'String', 'B. Release-related', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
    'FontSize', 10, ...
...
line([0 0], get(gca, 'ylim'), 'color', reward_col, 'linewidth', 1)
axis off
title('Unrewarded first poke (byTime)', 'fontname', 'dejavu sans', 'fontsize', 7)
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [col3-0.5 yshift_row6new 4 0.5], ...
    'String', 'D.  Activity vs time', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
    'FontSize', 10, ...
...
yshift_row7new = yshift_row6new+1.5;
%% add location
ha_loc = axes('unit', 'centimeters', 'position', [col3+0.5 yshift_row7new 2 3], 'nextplot', 'add');
...
title('Trigger/Press (byTime)', 'fontsize', 7);
axis off
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [col4-0.5 yshift_row3 5 1.2], ...
    'String', 'E. Trigger-related activity', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
    'FontSize', 10, ...
...
line(ha_press_psth, repmat(mean(ht_correct),[1 2]), FRrange, 'color', release_col, 'LineWidth', 1, 'linestyle','-.');
...
line(ha_press_psth_error, repmat(mean(premature_duration_presses),[1 2]), FRrange, 'color', release_col, 'linewidth', 1, 'linestyle','-.');

set(ha_release_psth, 'ylim', FRrange);
line(ha_release_psth, [0 0], FRrange, 'color', release_col, 'linewidth', 1);
line(ha_release_psth, repmat(-mean(ht_correct),[1 2]), FRrange, 'color', press_col, 'linewidth', 1, 'linestyle','-.');
line(ha_release_psth, repmat(mean(rmoutliers_custom(move_time)),[1 2]), FRrange, 'color', reward_col, 'linewidth', 1, 'linestyle','-.');

set(ha_release_psth_error, 'ylim', FRrange);
line(ha_release_psth_error, [0 0], FRrange, 'color', release_col, 'linewidth', 1);
line(ha_release_psth_error, repmat(-mean(premature_duration_presses),[1 2]), FRrange, 'color', press_col, 'linewidth', 1, 'linestyle','-.');
line(ha_release_psth_error, repmat(mean(rmoutliers_custom(move_time_nonreward_plot)),[1 2]), FRrange, 'color', reward_col, 'linewidth', 1, 'linestyle','-.');

set(ha_poke, 'ylim', FRrange);
line(ha_poke, [0 0], FRrange, 'color', reward_col, 'linewidth', 1);
line(ha_poke, repmat(-mean(rmoutliers_custom(move_time)),[1 2]), FRrange, 'color', release_col, 'linewidth', 1.5, 'linestyle','-.');
line(ha_poke, repmat(-mean(rmoutliers_custom(move_time_nonreward_plot)),[1 2]), FRrange, 'color', release_col.*1.2, 'linewidth', 1, 'linestyle','-.');

set(ha_trigger, 'ylim', FRrange);
line(ha_trigger, [0 0], FRrange, 'color', trigger_col, 'linewidth', 1);
line(ha_trigger, repmat(mean(ht_all),[1 2]), FRrange, 'color', release_col, 'linewidth', 1, 'linestyle','-.');
%% plot spks
col5=19.5;
thiscolor = [0 0 0];
