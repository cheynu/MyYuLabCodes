function PlotTrajectorySpikes_v2(sdf_unit, TrajTableAll, spatial_span, tReleases, TimeStamp, video_path, unit_name, type)
...
Trials = cell(length(trial_names), 5); % trial name, traj before press, traj to map before press, triaj after press, traj to map after press
...
for iTrial = 1:length(Trials)
    ...
    if ~isempty(ind_before_press)
         [map_out, xcenters, ycenters] = compute_corr2D(x_i(ind_before_press), y_i(ind_before_press), xBinEdges, yBinEdges);
         Trials{iTrial, 2} = [x_i(ind_before_press) y_i(ind_before_press)];
          Trials{iTrial, 3} = {map_out, xcenters, ycenters};
    end
    if ~isempty(ind_after_press)
        Trials{iTrial, 3} = [x_i(ind_after_press) y_i(ind_after_press)];
    end
end
...
annotation('textbox', 'units', 'centimeters', ...
    'position', [x_pos-.5 y_pos+plotSize(2)+0.25 .5 .5], ...
    'String', 'D_1', 'EdgeColor', 'none', ...
    'Interpreter', 'tex',...
    'HorizontalAlignment', 'center', 'FontSize', 10,...
    'fontweight', 'bold');
...
grid on
% only plot plotRatio of the data to reduce figure size
...
a               =   FR_range(1);
b               =   FR_range(2);
% Create custom colormap (using jet instead of turbo)
n_colors = 256;  % Number of colors in colormap
custom_map = hot(n_colors);

% Normalize z values to [0 255] range
z_low = sdf_values(sampled_low_indices);
z_high = sdf_values(sampled_high_indices);

z_normalized_low = (z_low - a) / (b - a);              % Normalize to [0,1]
z_normalized_low = max(0, min(1, z_normalized_low));   % Clamp between 0 and 1
color_indices_low = round(z_normalized_low * (n_colors-1)) + 1;  % Convert to color indices
% Convert indices to   colors
point_colors_low = custom_map(color_indices_low, :);
 
z_normalized_high = (z_high - a) / (b - a);              % Normalize to [0,1]
z_normalized_high= max(0, min(1, z_normalized_high));   % Clamp between 0 and 1
color_indices_high = round(z_normalized_high * (n_colors-1)) + 1;  % Convert to color indices
% Convert indices to   colors
point_colors_high = custom_map(color_indices_high, :);
 
marker_size = 20;
alpha_value = 0.5;
% Plot the randomly sampled points for hsc1 (before high firing)
hsc1 = scatter(ha_d1, x(sampled_low_indices), y(sampled_low_indices), marker_size, '.', 'CData', point_colors_low,'MarkerEdgeAlpha', alpha_value);
% Plot the randomly sampled points for hsc2 (after high firing)
hsc2 = scatter(ha_d1, x(sampled_high_indices), y(sampled_high_indices), marker_size, '.', 'CData', point_colors_high,'MarkerEdgeAlpha', alpha_value);
...
% Plot the randomly sampled points for hsc1 (before high firing)
hsc3 = scatter(ha_d2, x(sampled_low_indices), y(sampled_low_indices), marker_size, '.', 'CData', point_colors_low,'MarkerEdgeAlpha', alpha_value);
% Plot the randomly sampled points for hsc2 (after high firing)
hsc4 = scatter(ha_d2, x(sampled_high_indices), y(sampled_high_indices), marker_size, '.', 'CData', point_colors_high,'MarkerEdgeAlpha', alpha_value);
...
% This is panelg G1
annotation('textbox', 'units', 'centimeters', ...
    'position', [x_pos-1.5 y_pos+plotSize(2)+0.5 plotSize(1)*1.5  .5], ...
    'String', 'G_1 | Before press', 'EdgeColor', 'none', ...
    'Interpreter', 'tex',...
    'HorizontalAlignment', 'center', 'FontSize', 10,...
    'fontweight', 'bold');
ha_g1=axes;
...
annotation('textbox', 'units', 'centimeters', ...
    'position', [x_pos-1.5 y_pos+plotSize(2)+0.5 plotSize(1)*1.5  .5], ...
    'String', 'G_2 | After press', 'EdgeColor', 'none', ...
    'Interpreter', 'tex',...
    'HorizontalAlignment', 'center', 'FontSize', 10,...
    'fontweight', 'bold');
ha_g2=axes;
...
DataOut.SpikeRate2D = {'xBinEdges', 'yBinEdges', 'smoothedRateGridBefore', 'smoothedRateGridAfter'; xBinEdges, yBinEdges, smoothedRateGridBefore, smoothedRateGridAfter};
...
for i =1:2:length(x_bins)
    if ~isnan(y_means_to(i))
        line(ha_g1, [x_bins(i) x_bins(i)], y_ci_to(:, i), 'color', to_color);
        scatter(ha_g1, x_bins(i), y_means_to(i), '.', 'filled', 'markerfacecolor', 'none', 'markeredgecolor', to_color);
    end
    if ~isnan(y_means_from(i))
        line(ha_g2, [x_bins(i) x_bins(i)], y_ci_from(:, i), 'color', from_color);
        scatter(ha_g2, x_bins(i), y_means_from(i), '.', 'markerfacecolor', 'none', 'markeredgecolor', from_color);
    end
end
...
save(tosavename, 'DataOut')
    ...
    function [map_out, xcenters, ycenters] = compute_corr2D(x, y, xedges, yedges)
        % 
        % x_range = [0 800];
        % pix_size = 10;
        ...
        xcenters =xedges(1:end-1)+unique(diff(xedges));
        ycenters = yedges(1:end-1)+unique(diff(yedges));
