function PSTH = ComputePlotPSTHByTime2025(r, PSTHOut, ku, varargin)
...
% 2025/8 add probe trials
% 2025/8 plot the spike raster by time
...
for i =1:nFPs
    ...
    % order the trials by time
    [t_presses_correct{i}, ind_sort] = sort(t_presses_correct{i});
    trialspxmat_press{i} = trialspxmat_press{i}(:, ind_sort);
    rt_presses_sorted{i} =rt_presses_sorted{i}(ind_sort);
...
for i =1:nFPs
    ...
    % order the trials by time
    [t_releases_correct{i}, ind_sort] = sort(t_releases_correct{i});
    trialspxmat_release{i} = trialspxmat_release{i}(:, ind_sort);
    rt_releases_sorted{i} =rt_releases_sorted{i}(ind_sort);
...
FPs_premature_presses            =              PSTHOut.Presses.FP{nFPs+1};
...
% order the trials by time
[t_premature_presses, ind_sort] = sort(t_premature_presses);
trialspxmat_premature_press = trialspxmat_premature_press(:, ind_sort);
premature_duration_presses =premature_duration_presses(ind_sort);
...
FPs_premature_releases            =              PSTHOut.Releases.FP{nFPs+1};
...
% order the trials by time
[t_premature_releases, ind_sort] = sort(t_premature_releases);
trialspxmat_premature_release = trialspxmat_premature_release(:, ind_sort);
premature_duration_releases =premature_duration_releases(ind_sort);
...
FPs_late_presses            =              PSTHOut.Presses.FP{nFPs+2};
...
% order the trials by time
[t_late_presses, ind_sort] = sort(t_late_presses);
trialspxmat_late_press = trialspxmat_late_press(:, ind_sort);
late_duration_presses =late_duration_presses(ind_sort);
...
FPs_late_releases            =              PSTHOut.Releases.FP{nFPs+2};
...
% order the trials by time
[t_late_releases, ind_sort] = sort(t_late_releases);
trialspxmat_late_release = trialspxmat_late_release(:, ind_sort);
late_duration_releases =late_duration_releases(ind_sort);
...
% 2025: add probe
% probe press PSTH
t_probe_presses                 =         PSTHOut.Presses.Probe.Time;
probe_duration                  = PSTHOut.Presses.Probe.Duration;
[psth_probe_press, ts_probe_press, trialspxmat_probe_press, tspkmat_probe_press,...
    t_probe_presses, ind]        =           Spikes.jpsth(r.Units.SpikeTimes(ku).timings, ...
    t_probe_presses, params_press);
psth_probe_press             =             smoothdata (psth_probe_press, 'gaussian', 5);
probe_duration_presses = probe_duration(ind);

% order the trials by time
[t_probe_presses, ind_sort] = sort(t_probe_presses);
trialspxmat_probe_press = trialspxmat_probe_press(:, ind_sort);
probe_duration_presses =probe_duration_presses(ind_sort);

PSTH.ProbePresses = {psth_probe_press, ts_probe_press, trialspxmat_probe_press,...
    tspkmat_probe_press, t_probe_presses,probe_duration_presses};
PSTH.ProbeLabels = {'PSTH', 'tPSTH', 'SpikeMat', 'tSpikeMat', 'tEvents', 'HoldDuration'};

% probe release PSTH
t_probe_releases                 =         PSTHOut.Releases.Probe.Time;
probe_duration                  = PSTHOut.Releases.Probe.Duration;
[psth_probe_release, ts_probe_release, trialspxmat_probe_release, tspkmat_probe_release,...
    t_probe_releases, ind]        =           Spikes.jpsth(r.Units.SpikeTimes(ku).timings, ...
    t_probe_releases, params);
psth_probe_release             =             smoothdata (psth_probe_release, 'gaussian', 5);
probe_duration_releases = probe_duration(ind);
PSTH.ProbeReleases = {psth_probe_release, ts_probe_release, trialspxmat_probe_release,...
    tspkmat_probe_release, t_probe_releases, probe_duration_releases};
...
for i =1:length(t_reward_pokes)
    ...
    % order the trials by time
    [t_reward_pokes{i}, ind_sort] = sort(t_reward_pokes{i});
    trialspxmat_reward_pokes{i} = trialspxmat_reward_pokes{i}(:, ind_sort);
    move_time{i} =move_time{i}(ind_sort);
...
for i =1:nFPs
    ...
    psth_trigger_correct{i} = smoothdata (psth_trigger_correct{i}, 'gaussian', 5);
        ...
        % order the trials by time
    [t_triggers_correct{i}, ind_sort] = sort(t_triggers_correct{i});
    trialspxmat_trigger_correct{i} = trialspxmat_trigger_correct{i}(:, ind_sort);
    RT_triggers_correct{i} =RT_triggers_correct{i}(ind_sort);
...
hf=27;
figure(hf); clf(hf)
set(gcf, 'unit', 'centimeters', 'position', printsize, 'paperpositionmode', 'auto' ,'color', 'w', 'ToolBar','none')
...
hplot_press_probe = plot(ts_probe_press, psth_probe_press, 'color', probe_color,  'linewidth', .5);
FRMax = max([FRMax max(psth_probe_press)]);
...
line(xxrt_all, yyrt_all, 'color', release_col, 'linewidth', 2)
scatter(x_portin, y_portin, 8, 'o', 'filled','MarkerFaceColor', reward_col,  'markerfacealpha', 0.5, 'MarkerEdgeColor','none')
...
% Probe response raster plot
ntrial_probe = size(trialspxmat_probe_press, 2); % number of trials
axes('unit', 'centimeters', 'position', [1.25 yshift_row6  6 ntrial_probe*rasterheight],...
    'nextplot', 'add',...
    'xlim', [-PressTimeDomain(1) PressTimeDomain(2)], 'ylim', [-ntrial_probe 1], 'box', 'on');
yshift_row7             =      yshift_row6 + 0.5 + ntrial_late*rasterheight;
ap_mat          =     trialspxmat_probe_press;
t_mat             =     tspkmat_probe_press;
k =0;
xx_all = [];
yy_all = [];
xxrt_all = [];
...
for i =1:size(ap_mat, 2)
    iholddur =probe_duration_presses(i);
    xx =  t_mat(ap_mat(:, i)==1);
    yy1 = [0 0.8]-k;
    yy2 = [0 1]-k;
    xxrt = iholddur;
...
line(xx_all, yy_all, 'color', probe_color, 'linewidth', 1)
line(xxrt_all, yyrt_all, 'color', release_col, 'linewidth', 2)
...
if isfield(r, 'BehaviorClass')
    strings = [strings r.Meta(1).Subject '_'  r.BehaviorClass.Date];
else
    strings = [strings r.Meta(1).Subject '_'  strrep( r.Meta(1).DateTime(1:11), '-', '_')];
end

if size(r.Units.SpikeNotes, 2) == 4
    cluster_id = r.Units.SpikeNotes(ku, 4);
    strings = [strings  '_Ch' num2str(ch) '_Unit' num2str(unit_no) ' | ' 'Kilosort cluster ' num2str(cluster_id) ')'];
else
    strings = [strings  '_Ch' num2str(ch) '_Unit' num2str(unit_no) ')'];
end

annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [1, yshift_row8, 6, 1.2], ... % Normalize to figure units
    'String', strings, ...
...
annotation('textbox', ...
    'Units', 'centimeters', ... % Set units to centimeters
    'Position', [1+7, yshift_row8, 6, 1.2], ... % Normalize to figure units
    'String', 'Trials ordered by time', ...
    'Interpreter', 'none', ... % Prevent underscores from being treated as subscripts
    'BackgroundColor', 'w', ... % White background
    'FontSize', 10, ...
    'FontAngle','italic',...
...
% PSTH of probe trials
hplot_release_probe = plot(ts_probe_release, psth_probe_release, 'color', probe_color,  'linewidth', .5);
FRMax = max([FRMax max(psth_probe_release)]);
...
% Probe response raster plot
ntrial_probe = size(trialspxmat_probe_release, 2); % number of trials
axes('unit', 'centimeters', 'position', [8.25 yshift_row6  width ntrial_probe*rasterheight],...
    'nextplot', 'add',...
    'xlim', [-ReleaseTimeDomain(1) ReleaseTimeDomain(2)], 'ylim', [-ntrial_probe 1], 'box', 'on');
yshift_row7             =      yshift_row6 + 0.5 + ntrial_late*rasterheight;
ap_mat          =     trialspxmat_probe_release;
t_mat             =     tspkmat_probe_release;
k =0;
xx_all = [];
yy_all = [];
xxrt_all = [];
...
for i =1:size(ap_mat, 2)
    iholddur =probe_duration_releases(i);
    xx =  t_mat(ap_mat(:, i)==1);
    yy1 = [0 0.8]-k;
    yy2 = [0 1]-k;
    xxrt = -iholddur;
...
line(xx_all, yy_all, 'color', probe_color, 'linewidth', 1)
line(xxrt_all, yyrt_all, 'color', press_col, 'linewidth', 2)
...
% Add information
% uicontrol('Style','text','Units','centimeters','Position',[7.75 yshift_row7+0.5 width+1 1],...
%     'string', 'B. Release-related', ...
%     'FontName','Dejavu Sans', 'fontweight', 'bold','fontsize', 10,'BackgroundColor',[1 1 1],...
%     'HorizontalAlignment','Left');
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [7.75 yshift_row7+0.5 width 1], ...
    'String', 'B. Release-related', ...
    'Interpreter', 'none', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [col3-0.5 yshift_row4new 5 1.75], ...
    'String', 'C. Rewarded/Nonrewarded poke-related activity', ...
    'Interpreter', 'none', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [col3-0.5 yshift_row6new+0.2 4 0.3], ...
    'String', 'D. Activity vs time', ...
    'Interpreter', 'none', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [col4-0.5  yshift_row4 5 1.2], ...
    'String', 'E. Trigger-related activity', ...
    'Interpreter', 'none', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
...
if strcmpi(ToSave,'on')
    ...
    tosavename2= fullfile(thisFolder, [anm_name '_' session '_Ch'  num2str(ch) '_Unit' num2str(unit_no) '_ByTime']);
