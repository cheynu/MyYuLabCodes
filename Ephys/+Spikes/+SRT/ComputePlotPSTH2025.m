function PSTH = ComputePlotPSTH2025(r, PSTHOut, ku, varargin)
...
for i =1:length(t_reward_pokes)
    ...
    % sort the move time
    [move_time{i}, ind_sort] = sort(move_time{i});
    trialspxmat_reward_pokes{i} = trialspxmat_reward_pokes{i}(:, ind_sort);
    t_reward_pokes{i} = t_reward_pokes{i}(ind_sort);
...
for i =1:nFPs
    ...
    % sort the move time
    [RT_triggers_correct{i}, ind_sort] = sort(RT_triggers_correct{i});
    trialspxmat_trigger_correct{i} = trialspxmat_trigger_correct{i}(:, ind_sort);
    t_triggers_correct{i} = t_triggers_correct{i}(ind_sort);
...
annotation('textbox', ...
    'Units', 'centimeters', ...
    'Position', [7.75 yshift_row7+0.5 width+1 1], ...
    'String', 'B. Release-related', ...
    'Interpreter', 'none', ...
    'FontName', 'Dejavu Sans', ...
    'FontWeight', 'bold', ...
