function PSTH = ComputePlotPSTHWait26(r, ku, varargin)

% ComputePlotPSTHWait26  Compute and plot PSTHs/raster for SRT session with reward-history grouping.
%
...
FP_cols = [192, 127, 0; 76, 61, 61]/255; % warmup, normal
...
FP_types        =       {'WarmUp', 'Normal'}; % only for wait
nFPs            =       length(FP_types);

typesPressRelease = {};
FP_final = 1500;
r.EventTable.type (r.EventTable.FP == FP_final) = {'Normal'};

for i =1:nFPs

    switch FP_types{i}
        case 'WarmUp'
            iFP                     =   unique(r.EventTable.FP(strcmp(r.EventTable.type, FP_types{i})));
            type_                   =   'WarmUp';
        case 'Normal'
            fp_final                =   unique(r.EventTable.FP(strcmp(r.EventTable.type, 'Normal')));
            if numel(fp_final)>1
                fp_final = NaN;
            end
            type_                   =   'Normal';
    ...
    for j =1:length(beh_outcome)
        ...
        ind = strcmp(r.EventTable.type, FP_types{i}) & strcmp(r.EventTable.outcome, outcome_);
        fpj = r.EventTable.FP(ind);
        ...
        PSTH.Press.(type_).(outcome_).FP              =   fpj(ind_);
        ...
        PSTH.Release.(type_).(outcome_).FP              =   fpj(ind_);
        ...
        if strcmp(outcome_, 'Correct') || strcmp(outcome_, 'Late')
            ...
            PSTH.Trigger.(type_).(outcome_).FP              =   fpj(ind_);
        end
    end
end


% Specifically design rewarded versus unrewarded release: designed to study
% outcome-sensitive signals
types = {'Rewarded', 'Unrewarded'};
for i =1:length(types)
    itype                   =   types{i};
    switch itype
        case 'Rewarded' % correct trials
...
% Probe trials (There is no probe trials in 'Wait' protocol)
if false
...
for j =1:length(types)
    ...
    ind = strcmp(r.EventTable.outcome, 'Correct') & strcmp(r.EventTable.reward_level, type_);
    t_presses   =   r.EventTable.t_press(ind);
    t_releases  =   r.EventTable.t_release(ind);
    hold_dur    =   t_releases - t_presses;
    [psth_press, ts_press, trialspxmat_press, tspkmat_press, t_presses, ind_] = Spikes.jpsth(r.Units.SpikeTimes(ku).timings,...
    ...
    ind = ismember(r.EventTable.outcome, {'Correct'}) & strcmp(r.EventTable.reward_level, type_);
    t_trigger  =   r.EventTable.t_trigger(ind);
    t_release  =   r.EventTable.t_release(ind);
    rt          =   t_release - t_trigger;
    [psth_trigger, ts_trigger, trialspxmat_trigger, tspkmat_trigger, t_trigger, ind_] = Spikes.jpsth(r.Units.SpikeTimes(ku).timings,...
...
for j=1:length(poke_types)
    ...
    switch j_poke_type
        case 'Rewarded'
            ind = ~isnan(r.EventTable.t_poke) & strcmp(r.EventTable.outcome, 'Correct') & strcmp(r.EventTable.reward, 'Rewarded');
        ...
        case 'Omitted'
            ind = []; % no omitted design for wait task
...
glm_mdl = [];
if any(strcmp(spk_table.RewardType, 'Omitted'))
...
n_min = 20;                 % you already have this
% ---------- Press ----------
FRMaxPress = 5;  % floor
% Correct
...
FRMax = 1*max([FRMaxPress FRMaxRelease FRMaxPoke FRMaxTrigger]);
...
for i =1:nFPs
    ...
    if i == 1
        xline(ha_press_psth, mean(PSTH.Press.(field).Correct.FP), 'color', trigger_col, 'linestyle', ':', 'linewidth', 1);
    else
        xline(ha_press_psth, mean(PSTH.Press.(field).Correct.FP), 'color', trigger_col, 'linestyle', '-.', 'linewidth', 1);
    end
end
...
if by_time == 0
    ...
    for m =1:nFPs
        ...
        fp_dur = PSTH.Press.(field).Correct.FP;
    ...
    for m =1:nFPs
        ...
        fp_dur = PSTH.Release.(field).Correct.FP;
    ...
    for m =1:nFPs
        ...
        fp_color = trigger_col;
        fp_dur = PSTH.Press.(field).Premature.FP;
        [h, k] = Spikes.plotRasterFast(ax, ap_mat, t_mat, k, hold_dur, fp_dur,fp_color);
    end
    ...
    for m =1:nFPs
        ...
        fp_color = trigger_col;
        fp_dur = PSTH.Press.(field).Late.FP;
        [h, k] = Spikes.plotRasterFast(ax, ap_mat, t_mat, k, hold_dur, fp_dur, fp_color);
    end
    ...
    xline(0, 'color', press_col, 'linewidth', 1);
    ax = gca;  % or the handle returned from axes(...)
    h = addAxisCornerLabel(ax, sprintf('%s(n=%d)','All', length(fp_mat)), 'corner', 'topleft');
    axis off
...
pos = [0.1 yshift_row7+0.6 12 0.5];
annotation('textbox', ...
    'Units','centimeters', ...
    'Position',pos, ...
    'String',headerStr, ...
...
if by_time == 0
    ...
    axis off
    % Add Omitted trials (not available for wait task)
    yshift_row3 = yshift_row3+n*rasterheight+vspacing;

    if isempty(PSTH.Poke.Omitted)
        yshift_row4 = yshift_row3+vspacing;
    else
...
for m =1:nFPs
    ...
    n_trial = size(ap_mat, 2);
    rts = PSTH.Trigger.(field).Correct.ReactionTime;
    % sort rts (if needed)
    if by_time == 0
        [rts, ind_sort] = sort(rts);
        ap_mat = ap_mat(:, ind_sort);
    end
    k0  = k;
    [h, k] = Spikes.plotRasterFast(ax, ap_mat, t_mat, k, rts);

    if m >1
        line([1 1]*xrange_trigger(1), [-k0 -k], 'color', trigger_col, 'linewidth', 2);
        text(xrange_trigger(1)-100, -k0, '1500 ms', 'fontsize', 8);
    end
...
for m =1:nFPs
    ...
    rts = PSTH.Trigger.(field).Late.ReactionTime;
    % sort hold_dur (if needed)
    if by_time == 0
        [rts, ind_sort] = sort(rts);
        ap_mat = ap_mat(:, ind_sort);
...
for i =1:length(types)
    ...
    indi = find(strcmp(tab.outcome, types{i}));
...
for i =1:nFPs
    t_press = tab.t_press(strcmp(tab.type, FP_types{i}))/1000;
    fps = tab.FP(strcmp(tab.type, FP_types{i}))/1000;
    
    if i>1 && ~isempty(fp_final) && ~isnan(fp_final)
        line(t_press(1)*[1 1], [0 fp_final/1000], 'color', 'm', 'linewidth', 1);
    end
    plot(ha_press_beh, t_press, fps, 'color', trigger_col, 'linestyle', '-', 'linewidth', 1.5);

    title(sprintf('%s', FP_types{i}), 'fontsize', 9);
    
    indi = find(strcmp(tab.type, FP_types{i}));
...
if any(tab.FP>=1000)
    ...
    ha_release_beh =  axes('unit', 'centimeters', ...
        'position', [xnow+1.25 yshift_row5+1.5 width_beh height_beh-0.5], 'nextplot', 'add', ...
        'xlim', [-1 1], 'ylim', [0 3],...
        'xtick', (-1:0.5:1), 'xticklabelrotation', 45);

    plotshaded([.1 1], [0 0;10 10], [0 .8 .1])

    obj = Behavior.SRT.learning_data(r.EventTable);
    tabS = obj.select();
    stats = obj.rtDistribution(tabS, ...
        'fp_min', 1000);

    f_density = stats.f_density;
    bins = stats.bins;
    ...
    integral_norm = stats.integral_01_1;
    g_fit = stats.g_fit;
    plot(ha_release_beh, bins, g_fit, 'r', 'linewidth', 1.5)

    sk = stats.skew;
    sigma_hat = stats.sigma;

    s = sprintf('integral=%2.2f\nsknewness=%2.2f\nsigma=%2.2f',...
         integral_norm, sk, sigma_hat);

    annotation('textbox', ...
        'Units','centimeters', ...
        'Position',[xnow+1 yshift_row5+1.1+height_beh width_beh+0.25 0.75], ...
        'String',s, ...
        'Interpreter','none', ...
        'FitBoxToText','off', ...
        'BackgroundColor','w', ...
...
%% Look at reward probability modulation or error trials (for wait, we haven't implemented reward probability design as of Jan2026)
xnow = 1.25;
ynow = 5.5;
...
if n_low >10
    ...
    annotation('textbox', ...
        ...
        'String', 'High (—,r) vs Low (⋯,m) reward history', ...
    ...
    types = {'High'}; % for press, trigger, poke, we only plot responses during correct trials. 
    line_styles = {'-'};
    annotation('textbox', ...
        'Units', 'centimeters', ... % Set units to centimeters
        'Position', [1, ynow+height_psth+0.5, 8, 0.7], ... % Normalize to figure units
...
for i =1:length(types_)
    ...
    spk = spk(:, randperm(size(spk, 2), min(n, size(spk, 2))));
    % [h, k1] = plotRasterFast(ax, ap_mat, t, k0, hold_dur, fp_dur, fp_color, tick_color)
