...
% if height(BpodEvents.GoodWait)==1
%   trigger_bpod =   BpodEvents.GoodWait*1000; % in ms
% else
% trigger_bpod = BpodEvents.GoodWait(2,:)*1000; % in ms
% end

% use all triggers
trigger_bpod =   BpodEvents.Triggers*1000; % in ms
...
if length(seqson)>=3
    Indout = findseqmatchWXN(seqmom, seqson, toprint, toprintname);
...
% add outcome 
EventOut.Outcome = BpodEvents.Outcomes(Indout);
